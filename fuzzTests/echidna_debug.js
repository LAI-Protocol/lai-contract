...
contract('Echidna debugger', async accounts => {
  ...
  before(async () => {
    ...
    laiToken = await LAIToken.at(await echidnaTester.laiToken())
  ...
  it.only('LAI balance', async () => {
    await echidnaTester.openTroveExt('0', '0', '4210965169908805439447313562489173090')

    const totalSupply = await laiToken.totalSupply();
    const gasPoolBalance = await laiToken.balanceOf(GAS_POOL_ADDRESS);
    const activePoolBalance = await activePool.getLAIDebt();
    const defaultPoolBalance = await defaultPool.getLAIDebt();
    const stabilityPoolBalance = await stabilityPool.getTotalLAIDeposits();
    const currentTrove = await echidnaTester.echidnaProxies(0);
    const troveBalance = laiToken.balanceOf(currentTrove);
