...
* 
*
*  --- Functionality added specific to the LAOToken ---
* 
* 1) Transfer protection: blacklist of addresses that are invalid recipients (i.e. core Liquity contracts) in external 
* transfer() and transferFrom() calls. The purpose is to protect users from losing tokens by mistakenly sending LAO directly to a Liquity
* core contract, when they should rather call the right function.
*
* 2) sendToLAOStaking(): callable only by Liquity core contracts, which move LAO tokens from user -> LAOStaking contract.
*
...
* -sendToLAOStaking() reverts when the multisig is the sender, blocking the multisig from staking its LAO.
* 
* After one year has passed since deployment of the LAOToken, the restrictions on multisig operations are lifted
* and the multisig has the same rights as any other address.
*/

contract LAOToken is CheckContract, ILAOToken {
    using SafeMath for uint256;

    // --- ERC20 Data ---

    string constant internal _NAME = "LAO";
    string constant internal _SYMBOL = "LAO";
    ...
    address public immutable laoStakingAddress;
        ...
        public 
    {
        ...
        // Allocate the remainder to the LAO Multisig: (100 - 2 - 32 - 1.33) million = 64.66 million
    ...
    function sendToLAOStaking(address _sender, uint256 _amount) external override {
        _requireCallerIsLAOStaking();
        if (_isFirstYear()) { _requireSenderIsNotMultisig(_sender); }  // Prevent the multisig from staking LAO
        _transfer(_sender, laoStakingAddress, _amount);
    }
    ...
    (
        ...
        override 
    {            
        require(deadline >= now, 'LAO: expired deadline');
        bytes32 digest = keccak256(abi.encodePacked('\x19\x01', 
                         domainSeparator(), keccak256(abi.encode(
                         _PERMIT_TYPEHASH, owner, spender, amount, 
                         _nonces[owner]++, deadline))));
        address recoveredAddress = ecrecover(digest, v, r, s);
        require(recoveredAddress == owner, 'LAO: invalid signature');
    ...
    function _requireValidRecipient(address _recipient) internal view {
        require(
            ...
            "LAO: Cannot transfer tokens directly to the LAO token contract or the zero address"
        );
        require(
            _recipient != communityIssuanceAddress &&
            _recipient != laoStakingAddress,
            "LAO: Cannot transfer tokens directly to the community issuance or staking contract"
        );
    }
    ...
    function _requireRecipientIsRegisteredLC(address _recipient) internal view {
        ...
        "LAOToken: recipient must be a LockupContract registered in the Factory");
    }

    function _requireSenderIsNotMultisig(address _sender) internal view {
        require(_sender != multisigAddress, "LAOToken: sender must not be the multisig");
    }

    function _requireCallerIsNotMultisig() internal view {
        require(!_callerIsMultisig(), "LAOToken: caller must not be the multisig");
    }

    function _requireCallerIsLAOStaking() internal view {
         require(msg.sender == laoStakingAddress, "LAOToken: caller must be the LAOStaking contract");
    }
