...
contract EchidnaTester {
    ...
    uint private MCR;
    uint private CCR;
    uint private LAI_GAS_COMPENSATION;
    ...
    constructor() public payable {
        ...
        troveManager.setAddresses(address(borrowerOperations), 
            ...
            address(priceFeedTestnet), address(laiToken),
        ...
        stabilityPool.setAddresses(address(borrowerOperations), 
            address(troveManager), address(activePool), address(laiToken),
        ...
        for (uint i = 0; i < NUMBER_OF_ACTORS; i++) {
            echidnaProxies[i] = new EchidnaProxy(troveManager, borrowerOperations, stabilityPool, laiToken);
        ...
        LAI_GAS_COMPENSATION = borrowerOperations.LAI_GAS_COMPENSATION();
        require(MCR > 0);
        require(CCR > 0);
    ...
    function redeemCollateralExt(
        ...
        uint actor = _i % NUMBER_OF_ACTORS;
        echidnaProxies[actor].redeemCollateralPrx(_LAIAmount, _firstRedemptionHint, _upperPartialRedemptionHint, _lowerPartialRedemptionHint, _partialRedemptionHintNICR, 0, 0);
    }
    ...
    function getAdjustedETH(uint actorBalance, uint _ETH, uint ratio) internal view returns (uint) {
        ...
        require(price > 0);
        uint minETH = ratio.mul(LAI_GAS_COMPENSATION).div(price);
        require(actorBalance > minETH);
        uint ETH = minETH + _ETH % (actorBalance - minETH);
        return ETH;
    }

    function getAdjustedLAI(uint ETH, uint _LAIAmount, uint ratio) internal view returns (uint) {
        uint price = priceFeedTestnet.getPrice();
        uint LAIAmount = _LAIAmount;
        uint compositeDebt = LAIAmount.add(LAI_GAS_COMPENSATION);
        uint ICR = LiquityMath._computeCR(ETH, compositeDebt, price);
        if (ICR < ratio) {
            compositeDebt = ETH.mul(price).div(ratio);
            LAIAmount = compositeDebt.sub(LAI_GAS_COMPENSATION);
        }
        return LAIAmount;
    }

    function openTroveExt(uint _i, uint _ETH, uint _LAIAmount) public payable {
        uint actor = _i % NUMBER_OF_ACTORS;
        ...
        uint LAIAmount = getAdjustedLAI(ETH, _LAIAmount, CCR);

        //console.log('ETH', ETH);
        //console.log('LAIAmount', LAIAmount);

        echidnaProxy.openTrovePrx(ETH, LAIAmount, address(0), address(0), 0);
    ...
    function openTroveRawExt(uint _i, uint _ETH, uint _LAIAmount, address _upperHint, address _lowerHint, uint _maxFee) public payable {
        uint actor = _i % NUMBER_OF_ACTORS;
        echidnaProxies[actor].openTrovePrx(_ETH, _LAIAmount, _upperHint, _lowerHint, _maxFee);
    }
    ...
    function withdrawLAIExt(uint _i, uint _amount, address _upperHint, address _lowerHint, uint _maxFee) external {
        uint actor = _i % NUMBER_OF_ACTORS;
        echidnaProxies[actor].withdrawLAIPrx(_amount, _upperHint, _lowerHint, _maxFee);
    }

    function repayLAIExt(uint _i, uint _amount, address _upperHint, address _lowerHint) external {
        uint actor = _i % NUMBER_OF_ACTORS;
        echidnaProxies[actor].repayLAIPrx(_amount, _upperHint, _lowerHint);
    }
    ...
    function adjustTroveExt(uint _i, uint _ETH, uint _collWithdrawal, uint _debtChange, bool _isDebtIncrease) external payable {
        ...
        if (_isDebtIncrease) {
            ...
            debtChange = getAdjustedLAI(ETH, uint(_debtChange), MCR);
        }
    ...
    function echidna_trove_properties() public view returns(bool) {
        ...
        while (currentTrove != address(0)) {
            ...
            // Minimum debt (gas compensation)
            if (troveManager.getTroveDebt(currentTrove) < LAI_GAS_COMPENSATION) {
                return false;
            }
            // Uncomment to check that the condition is meaningful
            //else return false;
    ...
    // Total LAI matches
    function echidna_LAI_global_balances() public view returns(bool) {
        uint totalSupply = laiToken.totalSupply();
        uint gasPoolBalance = laiToken.balanceOf(address(gasPool));

        uint activePoolBalance = activePool.getLAIDebt();
        uint defaultPoolBalance = defaultPool.getLAIDebt();
        if (totalSupply != activePoolBalance + defaultPoolBalance) {
            return false;
        }

        uint stabilityPoolBalance = stabilityPool.getTotalLAIDeposits();
        address currentTrove = sortedTroves.getFirst();
        uint trovesBalance;
        while (currentTrove != address(0)) {
            trovesBalance += laiToken.balanceOf(address(currentTrove));
            currentTrove = sortedTroves.getNext(currentTrove);
        }
