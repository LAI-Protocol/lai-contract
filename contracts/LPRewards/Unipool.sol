...
/*
 * On deployment a new Uniswap pool will be created for the pair LAI/ETH and its token will be set here.
 ...
 * Funds for rewards will only be added once, on deployment of LAO token,
 ...
 * in order to avoid getting LAO tokens locked.
 * That also means that the start time for the program will be the event that occurs first:
 * either LAO token contract is deployed, and therefore LAO tokens are minted to Unipool contract,
...
contract Unipool is LPTokenWrapper, Ownable, CheckContract, IUnipool {
        ...
        onlyOwner
    {
        ...
        _notifyRewardAmount(laoToken.getLpRewardsEntitlement(), _duration);
    ...
    function claimReward() public override {
        ...
        laoToken.transfer(msg.sender, reward);
    ...
    function _notifyRewardAmount(uint256 _reward, uint256 _duration) internal {
        assert(_reward > 0);
        assert(_reward == laoToken.balanceOf(address(this)));
