...
def liquidate_troves(troves, index, data):
  ...
  price_LAI_previous = data.loc[index-1,'Price_LAI']
  price_LAO_previous = data.loc[index-1,'price_LAO']
  ...
  liquidation_gain = ether_liquidated*price_ether_current - debt_liquidated*price_LAI_previous
  airdrop_gain = price_LAO_previous * quantity_LAO_airdrop
  ...
  if index <= day:
   return_stability = initial_return*(1+shock_return)
    ...
    return_stability = min(0.5, 365*(data.loc[index-day:index, 'liquidation_gain'].sum()+data.loc[index-day:index, 'airdrop_gain'].sum())/(price_LAI_previous*stability_pool_previous))
  else:
    return_stability = (365/30)*(data.loc[index-month:index, 'liquidation_gain'].sum()+data.loc[index-month:index, 'airdrop_gain'].sum())/(price_LAI_previous*stability_pool_previous)
  
  return[troves, return_stability, debt_liquidated, ether_liquidated, liquidation_gain, airdrop_gain, n_liquidate]

"""Close Troves"""

def close_troves(troves, index2, price_LAI_previous):
  ...
  elif price_LAI_previous >=1:
    number_closetroves = max(0, n_steady * (1+shock_closetroves))
  else:
    number_closetroves = max(0, n_steady * (1+shock_closetroves)) + beta*(1-price_LAI_previous)*n_troves
...
def adjust_troves(troves, index):
  ...
  #A part of the troves are adjusted by adjusting debt
    if p >= ratio:
      ...
      if check>2:
        ...
        issuance_LAI_adjust = issuance_LAI_adjust + rate_issuance * (supply_new - working_trove['Supply'])
  ...
  return[troves, issuance_LAI_adjust]

"""Open Troves"""

def open_troves(troves, index1, price_LAI_previous):
  ...
  elif price_LAI_previous <=1 + rate_issuance:
    number_opentroves = max(0, n_steady * (1+shock_opentroves))
  else:
    number_opentroves = max(0, n_steady * (1+shock_opentroves)) + alpha*(price_LAI_previous-rate_issuance-1)*n_troves
  ...
  return[troves, number_opentroves, issuance_LAI_open]
...
#Calculating Price
  ...
  price_LAI_previous = float(data['Price_LAI'][index-1])
  price_LAI_current= price_LAI_previous*((supply-stability_pool)/(liquidity_pool_previous*(drift_liquidity+shock_liquidity)))**(1/delta)
...
#Stabilizer
  ...
  if price_LAI_current > 1.1 + rate_issuance:
    ...
    supply_wanted=stability_pool+liquidity_pool_previous*(drift_liquidity+shock_liquidity)*((1.1+rate_issuance)/price_LAI_previous)**delta
  ...
  if price_LAI_current < 1 - rate_redemption:
    ...
    #supply_current = sum(troves['Supply'])
    supply_target=stability_pool+liquidity_pool_previous*(drift_liquidity+shock_liquidity)*((1-rate_redemption)/price_LAI_previous)**delta
    ...
    if supply_diff < redemption_ratio * liquidity_pool:
      ...
      price_LAI_current= price_LAI_previous * (liquidity_pool/(liquidity_pool_previous*(drift_liquidity+shock_liquidity)))**(1/delta)
  ...
  troves = troves.reset_index(drop=True)
  return[price_LAI_current, liquidity_pool, troves, issuance_LAI_stabilizer, redemption_fee, n_redempt, redemption_pool, n_open]
...
def LAO_market(index, data):
  quantity_LAO = (100000000/3)*(1-0.5**(index/period))
  ...
  return[price_LAO_current, annualized_earning, MC_LAO_current]

"""# Simulation Program"""

#Defining Initials
initials = {"Price_LAI":[1.00], "Price_Ether":[price_ether_initial], "n_open":[initial_open], "n_close":[0], "n_liquidate": [0], "n_redempt":[0],
            "n_troves":[initial_open], "stability":[0], "liquidity":[0], "redemption_pool":[0],
            "supply_LAI":[0],  "return_stability":[initial_return], "airdrop_gain":[0], "liquidation_gain":[0],  "issuance_fee":[0], "redemption_fee":[0],
            "price_LAO":[price_LAO_initial], "MC_LAO":[0], "annualized_earning":[0]}
...
result_open = open_troves(troves, 0, data['Price_LAI'][0])
troves = result_open[0]
issuance_LAI_open = result_open[2]
data.loc[0,'issuance_fee'] = issuance_LAI_open * initials["Price_LAI"][0]
data.loc[0,'supply_LAI'] = troves["Supply"].sum()
...
#close troves
  result_close = close_troves(troves, index, price_LAI_previous)
...
#adjust troves
  ...
  issuance_LAI_adjust = result_adjustment[1]

#open troves
  result_open = open_troves(troves, index, price_LAI_previous)
  troves = result_open[0]
...
#Calculating Price, Liquidity Pool, and Redemption
  ...
  price_LAI_current = result_price[0]
  liquidity_pool = result_price[1]
  troves = result_price[2]
  issuance_LAI_stabilizer = result_price[3]
...
#Summary
  issuance_fee = price_LAI_current * (issuance_LAI_adjust + issuance_LAI_open + issuance_LAI_stabilizer)
  n_troves = troves.shape[0]
  supply_LAI = troves['Supply'].sum()
  if index >= month:
    price_LAO.append(price_LAO_current)

  new_row = {"Price_LAI":float(price_LAI_current), "Price_Ether":float(price_ether_current), "n_open":float(n_open), "n_close":float(n_close),
             "n_liquidate":float(n_liquidate), "n_redempt": float(n_redempt), "n_troves":float(n_troves),
              "stability":float(stability_pool), "liquidity":float(liquidity_pool), "redemption_pool":float(redemption_pool), "supply_LAI":float(supply_LAI),
             ...
             "annualized_earning":float(annualized_earning), "MC_LAO":float(MC_LAO_current), "price_LAO":float(price_LAO_current)
             }
  data = data.append(new_row, ignore_index=True)
  if price_LAI_current < 0:
    break
...
fig.add_trace(
    go.Scatter(x=data.index/720, y=data['Price_LAI'], name="LAI Price"),
    secondary_y=False,
)
fig.add_trace(
...
fig.update_layout(
    title_text="Price Dynamics of LAI and Ether"
)
fig.update_xaxes(tick0=0, dtick=1, title_text="Month")
fig.update_yaxes(title_text="LAI Price", secondary_y=False)
...
fig.add_trace(
    go.Scatter(x=data.index/720, y=data['supply_LAI'], name="LAI Supply"),
    secondary_y=True,
)
fig.update_layout(
    title_text="Dynamics of Trove Numbers and LAI Supply"
)
fig.update_xaxes(tick0=0, dtick=1, title_text="Month")
fig.update_yaxes(title_text="Number of Troves", secondary_y=False)
fig.update_yaxes(title_text="LAI Supply", secondary_y=True)
fig.show()
...
fig = make_subplots(specs=[[{"secondary_y": True}]])
fig.add_trace(
    go.Scatter(x=data.index/720, y=data['price_LAO'], name="LAO Price"),
    secondary_y=False,
)
fig.add_trace(
    go.Scatter(x=data.index/720, y=data['MC_LAO'], name="LAO Market Cap"),
    secondary_y=True,
)
fig.update_layout(
    title_text="Dynamics of the Price and Market Cap of LAO"
)
fig.update_xaxes(tick0=0, dtick=1, title_text="Month")
fig.update_yaxes(title_text="LAO Price", secondary_y=False)
fig.update_yaxes(title_text="LAO Market Cap", secondary_y=True)
fig.show()
...
initials = {"Price_LAI":[1.00], "Price_Ether":[price_ether_initial], "n_open":[initial_open], "n_close":[0], "n_liquidate": [0], "n_redempt":[0],
            "n_troves":[initial_open], "stability":[0], "liquidity":[0], "redemption_pool":[0],
            "supply_LAI":[0],  "return_stability":[initial_return], "airdrop_gain":[0], "liquidation_gain":[0],  "issuance_fee":[0], "redemption_fee":[0],
            "price_LAO":[price_LAO_initial], "MC_LAO":[0], "annualized_earning":[0], "base_rate":[base_rate_initial]}
data2 = pd.DataFrame(initials)
troves2= pd.DataFrame({"Ether_Price":[], "Ether_Quantity":[], "CR_initial":[], 
              "Supply":[], "Rational_inattention":[], "CR_current":[]})
result_open = open_troves(troves2, 0, data2['Price_LAI'][0])
troves2 = result_open[0]
issuance_LAI_open = result_open[2]
data2.loc[0,'issuance_fee'] = issuance_LAI_open * initials["Price_LAI"][0]
data2.loc[0,'supply_LAI'] = troves2["Supply"].sum()
...
#exogenous ether price input
  ...
  price_LAI_previous = data2.loc[index-1,'Price_LAI']
  price_LAO_previous = data2.loc[index-1,'price_LAO']
...
#close troves
  result_close = close_troves(troves2, index, price_LAI_previous)
...
#open troves
  result_open = open_troves(troves2, index, price_LAI_previous)
  troves2 = result_open[0]
  n_open = result_open[1]  
  issuance_LAI_open = result_open[2]
...
#LAO Market
  result_LAO = LAO_market(index, data2)
  price_LAO_current = result_LAO[0]
  annualized_earning = result_LAO[1]
  MC_LAO_current = result_LAO[2]
...
fig.add_trace(
    go.Scatter(x=data2.index/720, y=data2['Price_LAI'], name="LAI Price New", line = dict(dash='dot')),
    secondary_y=False,
)
fig.update_layout(
    title_text="Price Dynamics of LAI and Ether"
...
fig.add_trace(
    go.Scatter(x=data2.index/720, y=data2['supply_LAI'], name="LAI Supply New", line = dict(dash='dot')),
    secondary_y=True,
)
fig.update_layout(
    title_text="Dynamics of Trove Numbers and LAI Supply"
...
fig.add_trace(
    go.Scatter(x=data2.index/720, y=data2['price_LAO'], name="LAO Price New", line = dict(dash='dot')),
    secondary_y=False,
)
fig.add_trace(
    go.Scatter(x=data2.index/720, y=data2['MC_LAO'], name="LAO Market Cap New", line = dict(dash='dot')),
    secondary_y=True,
)
fig.update_layout(
    title_text="Dynamics of the Price and Market Cap of LAO"
