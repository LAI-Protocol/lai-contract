...
async function main() {
  ...
  const LAOContracts = await dh.deployLAOContractsHardhat(
      ARBITRARY_ADDRESS, 
      ARBITRARY_ADDRESS,
      ARBITRARY_ADDRESS
    )
 ...
 const { troveManager, borrowerOperations, hintHelpers, sortedTroves, priceFeedTestnet } = coreContracts
  ...
  await dh.connectCoreContracts(coreContracts, LAOContracts)
  await dh.connectLAOContracts(LAOContracts)
  await dh.connectLAOContractsToCore(LAOContracts, coreContracts)
  ...
  const LAIAmount = toBN(toWei('2500')) // borrower wants to withdraw 2500 LAI
  ...
  const liquidationReserve = await troveManager.LAI_GAS_COMPENSATION()
  const expectedFee = await troveManager.getBorrowingFeeWithDecay(LAIAmount)
  
  // Total debt of the new trove = LAI amount drawn, plus fee, plus the liquidation reserve
  const expectedDebt = LAIAmount.add(expectedFee).add(liquidationReserve)
  ...
  await borrowerOperations.openTrove(maxFee, LAIAmount, upperHint, lowerHint, { value: ETHColl })

  // --- adjust trove --- 

  const collIncrease = toBN(toWei('1'))  // borrower wants to add 1 ETH
  const LAIRepayment = toBN(toWei('230')) // borrower wants to repay 230 LAI

  // Get trove's current debt and coll
  const {0: debt, 1: coll} = await troveManager.getEntireDebtAndColl(borrower)
  
  const newDebt = debt.sub(LAIRepayment)
  ...
  await borrowerOperations.adjustTrove(maxFee, 0, LAIRepayment, false, upperHint, lowerHint, {value: collIncrease})


  // --- RedeemCollateral ---

  // Get the redemptions hints from the deployed HintHelpers contract
  const redemptionhint = await hintHelpers.getRedemptionHints(LAIAmount, price, 50)

  const {0: firstRedemptionHint, 1: partialRedemptionNewICR, 2: truncatedLAIAmount} = redemptionhint
  ...
  */
  ...
  /* Finally, perform the on-chain redemption, passing the truncated LAI amount, the correct hints, and the expected
  * ICR of the final partially redeemed trove in the sequence. 
  */
  await troveManager.redeemCollateral(truncatedLAIAmount,
    firstRedemptionHint,
