...
contract('StabilityPool', async accounts => {
  ...
  describe("Stability Pool Mechanisms", async () => {
    ...
    // --- provideToSP() ---
    // increases recorded LAI at Stability Pool
    it("provideToSP(): increases the Stability Pool LAI balance", async () => {
      // --- SETUP --- Give Alice a least 200
      await openTrove({ extraLAIAmount: toBN(200), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      ...
      // check LAI balances after
      const stabilityPool_LAI_After = await stabilityPool.getTotalLAIDeposits()
      assert.equal(stabilityPool_LAI_After, 200)
    })
    ...
    it("provideToSP(): reduces the user's LAI balance by the correct amount", async () => {
      // --- SETUP --- Give Alice a least 200
      await openTrove({ extraLAIAmount: toBN(200), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      ...
      // check user's LAI balance change
      const alice_LAIBalance_After = await laiToken.balanceOf(alice)
      assert.equal(alice_LAIBalance_Before.sub(alice_LAIBalance_After), '200')
    })

    it("provideToSP(): increases totalLAIDeposits by correct amount", async () => {
      // --- SETUP ---

      // Whale opens Trove with 50 ETH, adds 2000 LAI to StabilityPool
      await openTrove({ extraLAIAmount: toBN(dec(2000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale } })
      await stabilityPool.provideToSP(dec(2000, 18), frontEnd_1, { from: whale })

      const totalLAIDeposits = await stabilityPool.getTotalLAIDeposits()
      assert.equal(totalLAIDeposits, dec(2000, 18))
    })
    ...
    it('provideToSP(): Correctly updates user snapshots of accumulated rewards per unit staked', async () => {
      ...
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale, value: dec(50, 'ether') } })
      const whaleLAI = await laiToken.balanceOf(whale)
      await stabilityPool.provideToSP(whaleLAI, frontEnd_1, { from: whale })

      // 2 Troves opened, each withdraws minimum debt
      await openTrove({ extraLAIAmount: 0, ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_1, } })
      await openTrove({ extraLAIAmount: 0, ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_2, } })

      // Alice makes Trove and withdraws 100 LAI
      await openTrove({ extraLAIAmount: toBN(dec(100, 18)), ICR: toBN(dec(5, 18)), extraParams: { from: alice, value: dec(50, 'ether') } })


      // price drops: defaulter's Troves fall below MCR, whale doesn't
      await priceFeed.setPrice(dec(105, 18));

      const SPLAI_Before = await stabilityPool.getTotalLAIDeposits()
      ...
      const SPLAI_After = await stabilityPool.getTotalLAIDeposits()
      assert.isTrue(SPLAI_After.lt(SPLAI_Before))
    ...
    it("provideToSP(), multiple deposits: updates user's deposit and snapshots", async () => {
      ...
      // 3 Troves opened. Two users withdraw 160 LAI each
      await openTrove({ extraLAIAmount: 0, ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_1, value: dec(50, 'ether') } })
      await openTrove({ extraLAIAmount: 0, ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_2, value: dec(50, 'ether') } })
      await openTrove({ extraLAIAmount: 0, ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_3, value: dec(50, 'ether') } })

      // --- TEST ---

      // Alice makes deposit #1: 150 LAI
      await openTrove({ extraLAIAmount: toBN(dec(250, 18)), ICR: toBN(dec(3, 18)), extraParams: { from: alice } })
      ...
      // 2 users with Trove with 180 LAI drawn are closed
      await troveManager.liquidate(defaulter_1, { from: owner })  // 180 LAI closed
      await troveManager.liquidate(defaulter_2, { from: owner }) // 180 LAI closed

      const alice_compoundedDeposit_1 = await stabilityPool.getCompoundedLAIDeposit(alice)
      ...
      // Bob withdraws LAI and deposits to StabilityPool
      await openTrove({ extraLAIAmount: toBN(dec(1000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      await stabilityPool.provideToSP(dec(427, 18), frontEnd_1, { from: alice })

      // Defaulter 3 Trove is closed
      await troveManager.liquidate(defaulter_3, { from: owner })

      const alice_compoundedDeposit_2 = await stabilityPool.getCompoundedLAIDeposit(alice)
    ...
    it("provideToSP(): reverts if user tries to provide more than their LAI balance", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale, value: dec(50, 'ether') } })

      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice, value: dec(50, 'ether') } })
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob, value: dec(50, 'ether') } })
      const aliceLAIbal = await laiToken.balanceOf(alice)
      const bobLAIbal = await laiToken.balanceOf(bob)

      // Alice, attempts to deposit 1 wei more than her balance

      const aliceTxPromise = stabilityPool.provideToSP(aliceLAIbal.add(toBN(1)), frontEnd_1, { from: alice })
      await assertRevert(aliceTxPromise, "revert")

      // Bob, attempts to deposit 235534 more than his balance

      const bobTxPromise = stabilityPool.provideToSP(bobLAIbal.add(toBN(dec(235534, 18))), frontEnd_1, { from: bob })
      await assertRevert(bobTxPromise, "revert")
    })

    it("provideToSP(): reverts if user tries to provide 2^256-1 LAI, which exceeds their balance", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale, value: dec(50, 'ether') } })
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice, value: dec(50, 'ether') } })
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob, value: dec(50, 'ether') } })
      ...
      // Alice attempts to deposit 2^256-1 LAI
      try {
    ...
    it("provideToSP(): reverts if cannot receive ETH Gain", async () => {
      ...
      await laiToken.transfer(nonPayable.address, dec(250, 18), { from: whale })

      // NonPayable makes deposit #1: 150 LAI
      ...
      // NonPayable tries to make deposit #2: 100LAI (which also attempts to withdraw ETH gain)
    ...
    it("provideToSP(): doesn't impact other users' deposits or ETH gains", async () => {
      ...
      await openTrove({ extraLAIAmount: toBN(dec(1000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraLAIAmount: toBN(dec(2000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      await openTrove({ extraLAIAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: carol } })
      ...
      // D opens a trove
      await openTrove({ extraLAIAmount: toBN(dec(300, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: dennis } })
      ...
      const alice_LAIDeposit_Before = (await stabilityPool.getCompoundedLAIDeposit(alice)).toString()
      const bob_LAIDeposit_Before = (await stabilityPool.getCompoundedLAIDeposit(bob)).toString()
      const carol_LAIDeposit_Before = (await stabilityPool.getCompoundedLAIDeposit(carol)).toString()
      ...
      //check non-zero LAI and ETHGain in the Stability Pool
      const LAIinSP = await stabilityPool.getTotalLAIDeposits()
      const ETHinSP = await stabilityPool.getETH()
      assert.isTrue(LAIinSP.gt(mv._zeroBN))
      assert.isTrue(ETHinSP.gt(mv._zeroBN))

      // D makes an SP deposit
      await stabilityPool.provideToSP(dec(1000, 18), frontEnd_1, { from: dennis })
      assert.equal((await stabilityPool.getCompoundedLAIDeposit(dennis)).toString(), dec(1000, 18))

      const alice_LAIDeposit_After = (await stabilityPool.getCompoundedLAIDeposit(alice)).toString()
      const bob_LAIDeposit_After = (await stabilityPool.getCompoundedLAIDeposit(bob)).toString()
      const carol_LAIDeposit_After = (await stabilityPool.getCompoundedLAIDeposit(carol)).toString()
      ...
      assert.equal(alice_LAIDeposit_Before, alice_LAIDeposit_After)
      assert.equal(bob_LAIDeposit_Before, bob_LAIDeposit_After)
      assert.equal(carol_LAIDeposit_Before, carol_LAIDeposit_After)
    ...
    it("provideToSP(): doesn't impact system debt, collateral or TCR", async () => {
      ...
      // D opens a trove
      await openTrove({ extraLAIAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: dennis } })
      ...
      const activeDebt_Before = (await activePool.getLAIDebt()).toString()
      const defaultedDebt_Before = (await defaultPool.getLAIDebt()).toString()
      ...
      const activeDebt_After = (await activePool.getLAIDebt()).toString()
      const defaultedDebt_After = (await defaultPool.getLAIDebt()).toString()
    ...
    it("provideToSP(): doesn't impact any troves, including the caller's trove", async () => {
      ...
      // D opens a trove
      await openTrove({ extraLAIAmount: toBN(dec(1000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: dennis } })
    ...
    it("provideToSP(): doesn't protect the depositor's trove from liquidation", async () => {
      ...
      assert.equal((await stabilityPool.getCompoundedLAIDeposit(bob)).toString(), dec(1000, 18))
    ...
    it("provideToSP(): providing 0 LAI reverts", async () => {
      // --- SETUP ---
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale, value: dec(50, 'ether') } })
      ...
      // A, B, C provides 100, 50, 30 LAI to SP
      await stabilityPool.provideToSP(dec(100, 18), frontEnd_1, { from: alice })
      ...
      const LAIinSP_Before = (await stabilityPool.getTotalLAIDeposits()).toString()

      assert.equal(LAIinSP_Before, dec(180, 18))

      // Bob provides 0 LAI to the Stability Pool 
      const txPromise_B = stabilityPool.provideToSP(0, frontEnd_1, { from: bob })
      await th.assertRevert(txPromise_B)
    })

    // --- LAO functionality ---
    it("provideToSP(), new deposit: when SP > 0, triggers LAO reward event - increases the sum G", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale, value: dec(50, 'ether') } })

      // A, B, C open troves and make Stability Pool deposits
      await openTrove({ extraLAIAmount: toBN(dec(1000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraLAIAmount: toBN(dec(2000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraLAIAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
    ...
    it("provideToSP(), new deposit: sets the correct front end tag", async () => {
      ...
      await openTrove({ extraLAIAmount: toBN(dec(2000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraLAIAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraLAIAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
    ...
    it("provideToSP(), new deposit: depositor does not receive any LAO rewards", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale, value: dec(50, 'ether') } })
      ...
      // Get A, B, C LAO balances before and confirm they're zero
      const A_LAOBalance_Before = await laoToken.balanceOf(A)
      const B_LAOBalance_Before = await laoToken.balanceOf(B)

      assert.equal(A_LAOBalance_Before, '0')
      assert.equal(B_LAOBalance_Before, '0')
      ...
      // Get A, B, C LAO balances after, and confirm they're still zero
      const A_LAOBalance_After = await laoToken.balanceOf(A)
      const B_LAOBalance_After = await laoToken.balanceOf(B)

      assert.equal(A_LAOBalance_After, '0')
      assert.equal(B_LAOBalance_After, '0')
    })

    it("provideToSP(), new deposit after past full withdrawal: depositor does not receive any LAO rewards", async () => {
      ...
      await openTrove({ extraLAIAmount: toBN(dec(2000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraLAIAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraLAIAmount: toBN(dec(4000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })

      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_1 } })

      // --- SETUP --- 

      const initialDeposit_A = await laiToken.balanceOf(A)
      const initialDeposit_B = await laiToken.balanceOf(B)
      // A, B provide to SP
      ...
      // Get A, B, C LAO balances before and confirm they're non-zero
      const A_LAOBalance_Before = await laoToken.balanceOf(A)
      const B_LAOBalance_Before = await laoToken.balanceOf(B)
      assert.isTrue(A_LAOBalance_Before.gt(toBN('0')))
      assert.isTrue(B_LAOBalance_Before.gt(toBN('0')))
      ...
      // Get A, B, C LAO balances after, and confirm they have not changed
      const A_LAOBalance_After = await laoToken.balanceOf(A)
      const B_LAOBalance_After = await laoToken.balanceOf(B)

      assert.isTrue(A_LAOBalance_After.eq(A_LAOBalance_Before))
      assert.isTrue(B_LAOBalance_After.eq(B_LAOBalance_Before))
    })

    it("provideToSP(), new eligible deposit: tagged front end receives LAO rewards", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      await openTrove({ extraLAIAmount: toBN(dec(2000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraLAIAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraLAIAmount: toBN(dec(1000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({ extraLAIAmount: toBN(dec(2000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      await openTrove({ extraLAIAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: F } })
      ...
      // Get F1, F2, F3 LAO balances before, and confirm they're zero
      const frontEnd_1_LAOBalance_Before = await laoToken.balanceOf(frontEnd_1)
      const frontEnd_2_LAOBalance_Before = await laoToken.balanceOf(frontEnd_2)
      const frontEnd_3_LAOBalance_Before = await laoToken.balanceOf(frontEnd_3)

      assert.equal(frontEnd_1_LAOBalance_Before, '0')
      assert.equal(frontEnd_2_LAOBalance_Before, '0')
      assert.equal(frontEnd_3_LAOBalance_Before, '0')

      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_HOUR, web3.currentProvider)

      // console.log(`LAOSupplyCap before: ${await communityIssuance.LAOSupplyCap()}`)
      // console.log(`totalLAOIssued before: ${await communityIssuance.totalLAOIssued()}`)
      // console.log(`LAO balance of CI before: ${await laoToken.balanceOf(communityIssuance.address)}`)
      ...
      // console.log(`LAOSupplyCap after: ${await communityIssuance.LAOSupplyCap()}`)
      // console.log(`totalLAOIssued after: ${await communityIssuance.totalLAOIssued()}`)
      // console.log(`LAO balance of CI after: ${await laoToken.balanceOf(communityIssuance.address)}`)

      // Get F1, F2, F3 LAO balances after, and confirm they have increased
      const frontEnd_1_LAOBalance_After = await laoToken.balanceOf(frontEnd_1)
      const frontEnd_2_LAOBalance_After = await laoToken.balanceOf(frontEnd_2)
      const frontEnd_3_LAOBalance_After = await laoToken.balanceOf(frontEnd_3)

      assert.isTrue(frontEnd_1_LAOBalance_After.gt(frontEnd_1_LAOBalance_Before))
      assert.isTrue(frontEnd_2_LAOBalance_After.gt(frontEnd_2_LAOBalance_Before))
      assert.isTrue(frontEnd_3_LAOBalance_After.gt(frontEnd_3_LAOBalance_Before))
    })
    ...
    it("provideToSP(), new deposit: depositor does not receive ETH gains", async () => {
      ...
      // Whale transfers LAI to A, B
      await laiToken.transfer(A, dec(100, 18), { from: whale })
      await laiToken.transfer(B, dec(200, 18), { from: whale })

      // C, D open troves
      await openTrove({ extraLAIAmount: toBN(dec(1000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraLAIAmount: toBN(dec(2000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
    ...
    it("provideToSP(), new deposit after past full withdrawal: depositor does not receive ETH gains", async () => {
      ...
      // Whale transfers LAI to A, B
      await laiToken.transfer(A, dec(1000, 18), { from: whale })
      await laiToken.transfer(B, dec(1000, 18), { from: whale })

      // C, D open troves
      await openTrove({ extraLAIAmount: toBN(dec(4000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraLAIAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
    ...
    it("provideToSP(), topup: triggers LAO reward event - increases the sum G", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      // Expect G has increased from the LAO reward event triggered by B's topup
      assert.isTrue(G_After.gt(G_Before))
    })
    ...
    it("provideToSP(), topup from different front end: doesn't change the front end tag", async () => {
      ...
      await laiToken.transfer(D, dec(100, 18), { from: whale })
      await laiToken.transfer(E, dec(200, 18), { from: whale })

      // A, B, C open troves 
      await openTrove({ extraLAIAmount: toBN(dec(100, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraLAIAmount: toBN(dec(200, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraLAIAmount: toBN(dec(300, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
    ...
    it("provideToSP(), topup: depositor receives LAO rewards", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      // Get A, B, C LAO balance before
      const A_LAOBalance_Before = await laoToken.balanceOf(A)
      const B_LAOBalance_Before = await laoToken.balanceOf(B)
      const C_LAOBalance_Before = await laoToken.balanceOf(C)
      ...
      // Get LAO balance after
      const A_LAOBalance_After = await laoToken.balanceOf(A)
      const B_LAOBalance_After = await laoToken.balanceOf(B)
      const C_LAOBalance_After = await laoToken.balanceOf(C)

      // Check LAO Balance of A, B, C has increased
      assert.isTrue(A_LAOBalance_After.gt(A_LAOBalance_Before))
      assert.isTrue(B_LAOBalance_After.gt(B_LAOBalance_Before))
      assert.isTrue(C_LAOBalance_After.gt(C_LAOBalance_Before))
    })

    it("provideToSP(), topup: tagged front end receives LAO rewards", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      // Get front ends' LAO balance before
      const F1_LAOBalance_Before = await laoToken.balanceOf(frontEnd_1)
      const F2_LAOBalance_Before = await laoToken.balanceOf(frontEnd_2)
      const F3_LAOBalance_Before = await laoToken.balanceOf(frontEnd_3)
      ...
      // Get front ends' LAO balance after
      const F1_LAOBalance_After = await laoToken.balanceOf(A)
      const F2_LAOBalance_After = await laoToken.balanceOf(B)
      const F3_LAOBalance_After = await laoToken.balanceOf(C)

      // Check LAO Balance of front ends has increased
      assert.isTrue(F1_LAOBalance_After.gt(F1_LAOBalance_Before))
      assert.isTrue(F2_LAOBalance_After.gt(F2_LAOBalance_Before))
      assert.isTrue(F3_LAOBalance_After.gt(F3_LAOBalance_Before))
    })
    ...
    it("provideToSP(), topup: tagged front end's stake increases", async () => {
      ...
      await openTrove({ extraLAIAmount: toBN(dec(200, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraLAIAmount: toBN(dec(300, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraLAIAmount: toBN(dec(100, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({ extraLAIAmount: toBN(dec(200, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      await openTrove({ extraLAIAmount: toBN(dec(300, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: F } })
    ...
    it("provideToSP(), topup: tagged front end's snapshots update", async () => {
      ...
      // A, B, C, open troves 
      await openTrove({ extraLAIAmount: toBN(dec(200, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraLAIAmount: toBN(dec(400, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraLAIAmount: toBN(dec(600, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      ...
      await stabilityPool.provideToSP(await laiToken.balanceOf(D), ZERO_ADDRESS, { from: D })
      ...
      // between topups, because some block.timestamp time passes (and LAO is issued) between ops
      const G1 = await stabilityPool.epochToScaleToG(currentScale, currentEpoch)
      await stabilityPool.provideToSP(deposit_A, frontEnd_1, { from: A })
    ...
    it("provideToSP(): reverts when amount is zero", async () => {
      ...
      // Whale transfers LAI to C, D
      await laiToken.transfer(C, dec(100, 18), { from: whale })
      await laiToken.transfer(D, dec(100, 18), { from: whale })
    ...
    it("provideToSP(): reverts if user is a registered front end", async () => {
      // C, D, E, F open troves 
      await openTrove({ extraLAIAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraLAIAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({ extraLAIAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      await openTrove({ extraLAIAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: F } })
    ...
    it("withdrawFromSP(): reverts when user has no active deposit", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(100, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraLAIAmount: toBN(dec(100, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
    ...
    it("withdrawFromSP(): partial retrieval - retrieves correct LAI amount and the entire ETH Gain, and updates deposit", async () => {
      // --- SETUP ---
      // Whale deposits 185000 LAI in StabilityPool
      await openTrove({ extraLAIAmount: toBN(dec(1, 24)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      // Alice makes deposit #1: 15000 LAI
      await openTrove({ extraLAIAmount: toBN(dec(15000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: alice } })
      await stabilityPool.provideToSP(dec(15000, 18), frontEnd_1, { from: alice })

      // price drops: defaulters' Troves fall below MCR, alice and whale Trove remain active
      await priceFeed.setPrice(dec(105, 18));

      // 2 users with Trove with 170 LAI drawn are closed
      const liquidationTX_1 = await troveManager.liquidate(defaulter_1, { from: owner })  // 170 LAI closed
      const liquidationTX_2 = await troveManager.liquidate(defaulter_2, { from: owner }) // 170 LAI closed

      const [liquidatedDebt_1] = await th.getEmittedLiquidationValues(liquidationTX_1)
      const [liquidatedDebt_2] = await th.getEmittedLiquidationValues(liquidationTX_2)

      // Alice LAILoss is ((15000/200000) * liquidatedDebt), for each liquidation
      const expectedLAILoss_A = (liquidatedDebt_1.mul(toBN(dec(15000, 18))).div(toBN(dec(200000, 18))))
        .add(liquidatedDebt_2.mul(toBN(dec(15000, 18))).div(toBN(dec(200000, 18))))

      const expectedCompoundedLAIDeposit_A = toBN(dec(15000, 18)).sub(expectedLAILoss_A)
      const compoundedLAIDeposit_A = await stabilityPool.getCompoundedLAIDeposit(alice)

      assert.isAtMost(th.getDifference(expectedCompoundedLAIDeposit_A, compoundedLAIDeposit_A), 100000)

      // Alice retrieves part of her entitled LAI: 9000 LAI
      await stabilityPool.withdrawFromSP(dec(9000, 18), { from: alice })

      const expectedNewDeposit_A = (compoundedLAIDeposit_A.sub(toBN(dec(9000, 18))))
    ...
    it("withdrawFromSP(): partial retrieval - leaves the correct amount of LAI in the Stability Pool", async () => {
      // --- SETUP ---
      // Whale deposits 185000 LAI in StabilityPool
      await openTrove({ extraLAIAmount: toBN(dec(1, 24)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await stabilityPool.provideToSP(dec(185000, 18), frontEnd_1, { from: whale })
      ...
      const SP_LAI_Before = await stabilityPool.getTotalLAIDeposits()
      assert.equal(SP_LAI_Before, dec(200000, 18))
      ...
      Expect LAI in SP = (200000 - liquidatedDebt_1 - liquidatedDebt_2 - 9000) */
      const expectedSPLAI = toBN(dec(200000, 18))
        .sub(toBN(liquidatedDebt_1))
        .sub(toBN(liquidatedDebt_2))
        .sub(toBN(dec(9000, 18)))

      const SP_LAI_After = (await stabilityPool.getTotalLAIDeposits()).toString()

      th.assertIsApproximatelyEqual(SP_LAI_After, expectedSPLAI)
    })

    it("withdrawFromSP(): full retrieval - leaves the correct amount of LAI in the Stability Pool", async () => {
      // --- SETUP ---
      // Whale deposits 185000 LAI in StabilityPool
      ...
      const LAIinSPBefore = await stabilityPool.getTotalLAIDeposits()

      // Alice retrieves all of her entitled LAI:
      await stabilityPool.withdrawFromSP(dec(15000, 18), { from: alice })

      const expectedLAIinSPAfter = LAIinSPBefore.sub(compoundedLAIDeposit_A)

      const LAIinSPAfter = await stabilityPool.getTotalLAIDeposits()
      assert.isAtMost(th.getDifference(expectedLAIinSPAfter, LAIinSPAfter), 100000)
    })
    ...
    it("withdrawFromSP(): it correctly updates the user's LAI and ETH snapshots of entitled reward per unit staked", async () => {
      // --- SETUP ---
      // Whale deposits 185000 LAI in StabilityPool
      await openTrove({ extraLAIAmount: toBN(dec(1000000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await stabilityPool.provideToSP(dec(185000, 18), frontEnd_1, { from: whale })
    ...
    it("withdrawFromSP(): decreases StabilityPool ETH", async () => {
      ...
      const liquidationTx_1 = await troveManager.liquidate(defaulter_1, { from: owner })  // 180 LAI closed
    ...
    it("withdrawFromSP(): All depositors are able to withdraw from the SP to their account", async () => {
      ...
      for (account of depositors) {
        await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: account } })
      ...
      const totalDeposits = (await stabilityPool.getTotalLAIDeposits()).toString()

      assert.isAtMost(th.getDifference(totalDeposits, '0'), 100000)
    })

    it("withdrawFromSP(): increases depositor's LAI token balance by the expected amount", async () => {
      // Whale opens trove 
      await openTrove({ extraLAIAmount: toBN(dec(100000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      const aliceBalBefore = await laiToken.balanceOf(alice)
      const bobBalBefore = await laiToken.balanceOf(bob)

      /* From an offset of 10000 LAI, each depositor receives
      LAILoss = 1666.6666666666666666 LAI

      and thus with a deposit of 10000 LAI, each should withdraw 8333.3333333333333333 LAI (in practice, slightly less due to rounding error)
      */

      // Price bounces back to $200 per ETH
      await priceFeed.setPrice(dec(200, 18))

      // Bob issues a further 5000 LAI from his trove 
      await borrowerOperations.withdrawLAI(th._100pct, dec(5000, 18), bob, bob, { from: bob })

      // Expect Alice's LAI balance increase be very close to 8333.3333333333333333 LAI
      await stabilityPool.withdrawFromSP(dec(10000, 18), { from: alice })
      const aliceBalance = (await laiToken.balanceOf(alice))

      assert.isAtMost(th.getDifference(aliceBalance.sub(aliceBalBefore), '8333333333333333333333'), 100000)

      // expect Bob's LAI balance increase to be very close to  13333.33333333333333333 LAI
      await stabilityPool.withdrawFromSP(dec(10000, 18), { from: bob })
      const bobBalance = (await laiToken.balanceOf(bob))
    ...
    it("withdrawFromSP(): doesn't impact other users Stability deposits or ETH gains", async () => {
      ...
      await openTrove({ extraLAIAmount: toBN(dec(30000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: carol } })
    ...
    it("withdrawFromSP(): succeeds when amount is 0 and system has an undercollateralized trove", async () => {
      ...
      const A_LAOBalBefore = await laoToken.balanceOf(A)

      // Check Alice has gains to withdraw
      const A_pendingETHGain = await stabilityPool.getDepositorETHGain(A)
      const A_pendingLAOGain = await stabilityPool.getDepositorLAOGain(A)
      assert.isTrue(A_pendingETHGain.gt(toBN('0')))
      assert.isTrue(A_pendingLAOGain.gt(toBN('0')))
      ...
      const A_LAOBalAfter = await laoToken.balanceOf(A)
      const A_LAOBalDiff = A_LAOBalAfter.sub(A_LAOBalBefore)

      // Check A's ETH and LAO balances have increased correctly
      assert.isTrue(A_ETHBalAfter.sub(A_ETHBalBefore).eq(A_pendingETHGain))
      assert.isAtMost(th.getDifference(A_LAOBalDiff, A_pendingLAOGain), 1000)
    })

    it("withdrawFromSP(): withdrawing 0 LAI doesn't alter the caller's deposit or the total LAI in the Stability Pool", async () => {
      // --- SETUP ---
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      // Bob withdraws 0 LAI from the Stability Pool 
      await stabilityPool.withdrawFromSP(0, { from: bob })

      // check Bob's deposit and total LAI in Stability Pool has not changed
      const bob_Deposit_After = (await stabilityPool.getCompoundedLAIDeposit(bob)).toString()
      const LAIinSP_After = (await stabilityPool.getTotalLAIDeposits()).toString()

      assert.equal(bob_Deposit_Before, bob_Deposit_After)
      assert.equal(LAIinSP_Before, LAIinSP_After)
    })
    ...
    it("withdrawFromSP(): withdrawing 0 ETH Gain does not alter the caller's ETH balance, their trove collateral, or the ETH  in the Stability Pool", async () => {
      ...
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_1 } })
    ...
    it("withdrawFromSP(): Request to withdraw > caller's deposit only withdraws the caller's compounded deposit", async () => {
      ...
      const alice_LAI_Balance_Before = await laiToken.balanceOf(alice)
      const bob_LAI_Balance_Before = await laiToken.balanceOf(bob)

      const alice_Deposit_Before = await stabilityPool.getCompoundedLAIDeposit(alice)
      const bob_Deposit_Before = await stabilityPool.getCompoundedLAIDeposit(bob)

      const LAIinSP_Before = await stabilityPool.getTotalLAIDeposits()
      ...
      // Check Bob's LAI balance has risen by only the value of his compounded deposit
      const bob_expectedLAIBalance = (bob_LAI_Balance_Before.add(bob_Deposit_Before)).toString()
      const bob_LAI_Balance_After = (await laiToken.balanceOf(bob)).toString()
      assert.equal(bob_LAI_Balance_After, bob_expectedLAIBalance)

      // Alice attempts to withdraws 2309842309.000000000000000000 LAI from the Stability Pool 
      await stabilityPool.withdrawFromSP('2309842309000000000000000000', { from: alice })

      // Check Alice's LAI balance has risen by only the value of her compounded deposit
      const alice_expectedLAIBalance = (alice_LAI_Balance_Before.add(alice_Deposit_Before)).toString()
      const alice_LAI_Balance_After = (await laiToken.balanceOf(alice)).toString()
      assert.equal(alice_LAI_Balance_After, alice_expectedLAIBalance)

      // Check LAI in Stability Pool has been reduced by only Alice's compounded deposit and Bob's compounded deposit
      const expectedLAIinSP = (LAIinSP_Before.sub(alice_Deposit_Before).sub(bob_Deposit_Before)).toString()
      const LAIinSP_After = (await stabilityPool.getTotalLAIDeposits()).toString()
      assert.equal(LAIinSP_After, expectedLAIinSP)
    })

    it("withdrawFromSP(): Request to withdraw 2^256-1 LAI only withdraws the caller's compounded deposit", async () => {
      // --- SETUP ---
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      // Bob attempts to withdraws maxBytes32 LAI from the Stability Pool
      ...
      // Check LAI in Stability Pool has been reduced by only  Bob's compounded deposit
      const expectedLAIinSP = (LAIinSP_Before.sub(bob_Deposit_Before)).toString()
      const LAIinSP_After = (await stabilityPool.getTotalLAIDeposits()).toString()
      assert.equal(LAIinSP_After, expectedLAIinSP)
    })
    ...
    it("withdrawFromSP(): caller can withdraw full deposit and ETH gain during Recovery Mode", async () => {
      ...
      await openTrove({ extraLAIAmount: toBN(dec(1000000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale } })
      // Price halves
      await priceFeed.setPrice(dec(200, 18))

      // A, B, C open troves and make Stability Pool deposits
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(4, 18)), extraParams: { from: alice } })
      await openTrove({ extraLAIAmount: toBN(dec(20000, 18)), ICR: toBN(dec(4, 18)), extraParams: { from: bob } })
      await openTrove({ extraLAIAmount: toBN(dec(30000, 18)), ICR: toBN(dec(4, 18)), extraParams: { from: carol } })

      await borrowerOperations.openTrove(th._100pct, await getOpenTroveLAIAmount(dec(10000, 18)), defaulter_1, defaulter_1, { from: defaulter_1, value: dec(100, 'ether') })

      // A, B, C provides 10000, 5000, 3000 LAI to SP
      await stabilityPool.provideToSP(dec(10000, 18), frontEnd_1, { from: alice })
      ...
      const alice_LAI_Balance_Before = await laiToken.balanceOf(alice)
      const bob_LAI_Balance_Before = await laiToken.balanceOf(bob)
      const carol_LAI_Balance_Before = await laiToken.balanceOf(carol)
      ...
      const alice_Deposit_Before = await stabilityPool.getCompoundedLAIDeposit(alice)
      const bob_Deposit_Before = await stabilityPool.getCompoundedLAIDeposit(bob)
      const carol_Deposit_Before = await stabilityPool.getCompoundedLAIDeposit(carol)
      ...
      // Check LAI balances of A, B, C have risen by the value of their compounded deposits, respectively
      const alice_expectedLAIBalance = (alice_LAI_Balance_Before.add(alice_Deposit_Before)).toString()
     
      const bob_expectedLAIBalance = (bob_LAI_Balance_Before.add(bob_Deposit_Before)).toString()
      const carol_expectedLAIBalance = (carol_LAI_Balance_Before.add(carol_Deposit_Before)).toString()

      const alice_LAI_Balance_After = (await laiToken.balanceOf(alice)).toString()
 
      const bob_LAI_Balance_After = (await laiToken.balanceOf(bob)).toString()
      const carol_LAI_Balance_After = (await laiToken.balanceOf(carol)).toString()

      assert.equal(alice_LAI_Balance_After, alice_expectedLAIBalance)
      assert.equal(bob_LAI_Balance_After, bob_expectedLAIBalance)
      assert.equal(carol_LAI_Balance_After, carol_expectedLAIBalance)
      ...
      // Check LAI in Stability Pool has been reduced by A, B and C's compounded deposit
      const expectedLAIinSP = (LAIinSP_Before
    ...
    it("getDepositorETHGain(): depositor does not earn further ETH gains from liquidations while their compounded deposit == 0: ", async () => {
      ...
      await openTrove({ extraLAIAmount: toBN(dec(15000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_1 } })
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_2 } })
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_3 } })

      // A, B, provide 10000, 5000 LAI to SP
      await stabilityPool.provideToSP(dec(10000, 18), frontEnd_1, { from: alice })
      await stabilityPool.provideToSP(dec(5000, 18), frontEnd_1, { from: bob })
      ...
      assert.equal(LAIinSP, '0')

      // Check Stability deposits have been fully cancelled with debt, and are now all zero
      const alice_Deposit = (await stabilityPool.getCompoundedLAIDeposit(alice)).toString()
      const bob_Deposit = (await stabilityPool.getCompoundedLAIDeposit(bob)).toString()
      ...
      // Whale deposits 10000 LAI to Stability Pool
    ...
    // --- LAO functionality ---
    it("withdrawFromSP(): triggers LAO reward event - increases the sum G", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(1, 24)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })

      // A, B, C open troves
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraLAIAmount: toBN(dec(20000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraLAIAmount: toBN(dec(30000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
    ...
    it("withdrawFromSP(), partial withdrawal: depositor receives LAO rewards", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
    ...
    it("withdrawFromSP(), partial withdrawal: tagged front end receives LAO rewards", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
    ...
    it("withdrawFromSP(), partial withdrawal: tagged front end's stake decreases", async () => {
      ...
      await openTrove({ extraLAIAmount: toBN(dec(20000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraLAIAmount: toBN(dec(30000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({ extraLAIAmount: toBN(dec(20000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      await openTrove({ extraLAIAmount: toBN(dec(30000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: F } })
    ...
    it("withdrawFromSP(), partial withdrawal: tagged front end's snapshots update", async () => {
      ...
      await openTrove({ extraLAIAmount: toBN(dec(60000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })

      // D opens trove
      await openTrove({ extraLAIAmount: toBN(dec(100000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
    ...
    it("withdrawFromSP(), full withdrawal: removes deposit's front end tag", async () => {
      ...
      await laiToken.transfer(B, dec(20000, 18), { from: whale })
    ...
    it("withdrawFromSP(), full withdrawal: zero's depositor's snapshots", async () => {
      ...
      await openTrove({ extraLAIAmount: toBN(dec(20000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: E } })
      await stabilityPool.provideToSP(dec(10000, 18), frontEnd_3, { from: E })

      // Fast-forward time and make a second deposit, to trigger LAO reward and make G > 0
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_HOUR, web3.currentProvider)
      await stabilityPool.provideToSP(dec(10000, 18), frontEnd_3, { from: E })
      ...
      // C, D open troves
      await openTrove({ extraLAIAmount: toBN(dec(30000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: C } })
      await openTrove({ extraLAIAmount: toBN(dec(40000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: D } })
    ...
    it("withdrawFromSP(), full withdrawal that reduces front end stake to 0: zero’s the front end’s snapshots", async () => {
      ...
      // E opens trove and makes a deposit
      await openTrove({ extraLAIAmount: toBN(dec(30000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      await stabilityPool.provideToSP(dec(10000, 18), frontEnd_3, { from: E })
      ...
      // A, B open troves
      await openTrove({ extraLAIAmount: toBN(dec(30000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraLAIAmount: toBN(dec(40000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
    ...
    it("withdrawFromSP(), reverts when initial deposit value is 0", async () => {
      ...
      await openTrove({ extraLAIAmount: toBN(dec(10100, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await stabilityPool.provideToSP(dec(10000, 18), frontEnd_1, { from: A })

      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_1 } })

      //  SETUP: Execute a series of operations to trigger LAO and ETH rewards for depositor A
    ...
    it("withdrawETHGainToTrove(): Applies LAILoss to user's deposit, and redirects ETH reward to user's Trove", async () => {
      // --- SETUP ---
      // Whale deposits 185000 LAI in StabilityPool
      await openTrove({ extraLAIAmount: toBN(dec(1000000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await stabilityPool.provideToSP(dec(185000, 18), frontEnd_1, { from: whale })
      ...
      const compoundedDeposit_A = await stabilityPool.getCompoundedLAIDeposit(alice)
      ...
      const expectedLAILoss_A = liquidatedDebt.mul(toBN(dec(15000, 18))).div(toBN(dec(200000, 18)))
      const expectedCompoundedDeposit_A = toBN(dec(15000, 18)).sub(expectedLAILoss_A)
      ...
      // check Alice's LAILoss has been applied to her deposit expectedCompoundedDeposit_A
    ...
    it("withdrawETHGainToTrove(): reverts if user has no trove", async () => {
     ...
     await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_1 } })
      ...
      // A transfers LAI to D
      await laiToken.transfer(dennis, dec(10000, 18), { from: alice })
    ...
    it("withdrawETHGainToTrove(): triggers LAO reward event - increases the sum G", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
    ...
    it("withdrawETHGainToTrove(), eligible deposit: depositor receives LAO rewards", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
    ...
    it("withdrawETHGainToTrove(), eligible deposit: tagged front end receives LAO rewards", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
     ...
     await priceFeed.setPrice(dec(105, 18))
      ...
      // Get front ends' LAO balance after
      const F1_LAOBalance_After = await laoToken.balanceOf(frontEnd_1)
      const F2_LAOBalance_After = await laoToken.balanceOf(frontEnd_2)
      const F3_LAOBalance_After = await laoToken.balanceOf(frontEnd_3)
    ...
    it("withdrawETHGainToTrove(): reverts when depositor has no ETH gain", async () => {
      ...
      // fastforward time, and E makes a deposit, creating LAO rewards for all
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_HOUR, web3.currentProvider)
      await openTrove({ extraLAIAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
    ...
    it("registerFrontEnd(): reverts if address has a non-zero deposit already", async () => {
      // C, D, E open troves 
      await openTrove({ extraLAIAmount: toBN(dec(10, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraLAIAmount: toBN(dec(10, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({ extraLAIAmount: toBN(dec(10, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
