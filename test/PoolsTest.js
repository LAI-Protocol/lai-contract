...
contract('StabilityPool', async accounts => {
  ...
  */
  ...
  it('getTotalLAIDeposits(): gets the recorded LAI balance', async () => {
    const recordedETHBalance = await stabilityPool.getTotalLAIDeposits()
    assert.equal(recordedETHBalance, 0)
  })
})
...
contract('ActivePool', async accounts => {
  ...
  it('getLAIDebt(): gets the recorded LAI balance', async () => {
    const recordedETHBalance = await activePool.getLAIDebt()
    assert.equal(recordedETHBalance, 0)
  })
 
  it('increaseLAI(): increases the recorded LAI balance by the correct amount', async () => {
    const recordedLAI_balanceBefore = await activePool.getLAIDebt()
    assert.equal(recordedLAI_balanceBefore, 0)

    // await activePool.increaseLAIDebt(100, { from: mockBorrowerOperationsAddress })
    const increaseLAIDebtData = th.getTransactionData('increaseLAIDebt(uint256)', ['0x64'])
    const tx = await mockBorrowerOperations.forward(activePool.address, increaseLAIDebtData)
    assert.isTrue(tx.receipt.status)
    const recordedLAI_balanceAfter = await activePool.getLAIDebt()
    assert.equal(recordedLAI_balanceAfter, 100)
  })
  // Decrease
  it('decreaseLAI(): decreases the recorded LAI balance by the correct amount', async () => {
    // start the pool on 100 wei
    //await activePool.increaseLAIDebt(100, { from: mockBorrowerOperationsAddress })
    const increaseLAIDebtData = th.getTransactionData('increaseLAIDebt(uint256)', ['0x64'])
    const tx1 = await mockBorrowerOperations.forward(activePool.address, increaseLAIDebtData)
    assert.isTrue(tx1.receipt.status)

    const recordedLAI_balanceBefore = await activePool.getLAIDebt()
    assert.equal(recordedLAI_balanceBefore, 100)

    //await activePool.decreaseLAIDebt(100, { from: mockBorrowerOperationsAddress })
    const decreaseLAIDebtData = th.getTransactionData('decreaseLAIDebt(uint256)', ['0x64'])
    const tx2 = await mockBorrowerOperations.forward(activePool.address, decreaseLAIDebtData)
    assert.isTrue(tx2.receipt.status)
    const recordedLAI_balanceAfter = await activePool.getLAIDebt()
    assert.equal(recordedLAI_balanceAfter, 0)
  })
...
contract('DefaultPool', async accounts => {
  ...
  it('getETH(): gets the recorded LAI balance', async () => {
    const recordedETHBalance = await defaultPool.getETH()
    assert.equal(recordedETHBalance, 0)
  })

  it('getLAIDebt(): gets the recorded LAI balance', async () => {
    const recordedETHBalance = await defaultPool.getLAIDebt()
    assert.equal(recordedETHBalance, 0)
  })
 
  it('increaseLAI(): increases the recorded LAI balance by the correct amount', async () => {
    const recordedLAI_balanceBefore = await defaultPool.getLAIDebt()
    assert.equal(recordedLAI_balanceBefore, 0)

    // await defaultPool.increaseLAIDebt(100, { from: mockTroveManagerAddress })
    const increaseLAIDebtData = th.getTransactionData('increaseLAIDebt(uint256)', ['0x64'])
    const tx = await mockTroveManager.forward(defaultPool.address, increaseLAIDebtData)
    assert.isTrue(tx.receipt.status)

    const recordedLAI_balanceAfter = await defaultPool.getLAIDebt()
    assert.equal(recordedLAI_balanceAfter, 100)
  })
  ...
  it('decreaseLAI(): decreases the recorded LAI balance by the correct amount', async () => {
    ...
    //await defaultPool.increaseLAIDebt(100, { from: mockTroveManagerAddress })
    const increaseLAIDebtData = th.getTransactionData('increaseLAIDebt(uint256)', ['0x64'])
    const tx1 = await mockTroveManager.forward(defaultPool.address, increaseLAIDebtData)
    assert.isTrue(tx1.receipt.status)

    const recordedLAI_balanceBefore = await defaultPool.getLAIDebt()
    assert.equal(recordedLAI_balanceBefore, 100)

    // await defaultPool.decreaseLAIDebt(100, { from: mockTroveManagerAddress })
    const decreaseLAIDebtData = th.getTransactionData('decreaseLAIDebt(uint256)', ['0x64'])
    const tx2 = await mockTroveManager.forward(defaultPool.address, decreaseLAIDebtData)
    assert.isTrue(tx2.receipt.status)
