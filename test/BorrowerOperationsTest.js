...
const LAITokenTester = artifacts.require("./LAITokenTester")
...
/* NOTE: Some of the borrowing tests do not test for specific LAI fee values. They only test that the
 * fees are non-zero when they should occur, and that they decay over time.
 *
 * Specific LAI fee values will depend on the final fee schedule used, and the final choice for
...
contract('BorrowerOperations', async accounts => {
  ...
  const testCorpus = ({ withProxy = false }) => {
    ...
    it("addColl(), active Trove: applies pending rewards and updates user's L_ETH, L_LAIDebt snapshots", async () => {
      // --- SETUP ---

      const { collateral: aliceCollBefore, totalDebt: aliceDebtBefore } = await openTrove({ extraLAIAmount: toBN(dec(15000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      const { collateral: bobCollBefore, totalDebt: bobDebtBefore } = await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      await openTrove({ extraLAIAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: carol } })

      // --- TEST ---

      // price drops to 1ETH:100LAI, reducing Carol's ICR below MCR
      ...
      const L_LAIDebt = await troveManager.L_LAIDebt()
      ...
      const alice_LAIDebtRewardSnapshot_Before = alice_rewardSnapshot_Before[1]
      ...
      const bob_LAIDebtRewardSnapshot_Before = bob_rewardSnapshot_Before[1]

      assert.equal(alice_ETHrewardSnapshot_Before, 0)
      assert.equal(alice_LAIDebtRewardSnapshot_Before, 0)
      assert.equal(bob_ETHrewardSnapshot_Before, 0)
      assert.equal(bob_LAIDebtRewardSnapshot_Before, 0)
      ...
      const alicePendingLAIDebtReward = await troveManager.getPendingLAIDebtReward(alice)
      const bobPendingLAIDebtReward = await troveManager.getPendingLAIDebtReward(bob)
      for (reward of [alicePendingETHReward, bobPendingETHReward, alicePendingLAIDebtReward, bobPendingLAIDebtReward]) {
      ...
      assert.isTrue(aliceNewDebt.eq(aliceDebtBefore.add(alicePendingLAIDebtReward)))
      assert.isTrue(bobNewColl.eq(bobCollBefore.add(bobPendingETHReward).add(bobTopUp)))
      assert.isTrue(bobNewDebt.eq(bobDebtBefore.add(bobPendingLAIDebtReward)))

      /* Check that both Alice and Bob's snapshots of the rewards-per-unit-staked metrics should be updated
       to the latest values of L_ETH and L_LAIDebt */
      ...
      const alice_LAIDebtRewardSnapshot_After = alice_rewardSnapshot_After[1]
      ...
      const bob_LAIDebtRewardSnapshot_After = bob_rewardSnapshot_After[1]

      assert.isAtMost(th.getDifference(alice_ETHrewardSnapshot_After, L_ETH), 100)
      assert.isAtMost(th.getDifference(alice_LAIDebtRewardSnapshot_After, L_LAIDebt), 100)
      assert.isAtMost(th.getDifference(bob_ETHrewardSnapshot_After, L_ETH), 100)
      assert.isAtMost(th.getDifference(bob_LAIDebtRewardSnapshot_After, L_LAIDebt), 100)
    })
    ...
    //   // --- SETUP ---
    //   // A,B,C add 15/5/5 ETH, withdraw 100/100/900 LAI
    ...
    //   // close Carol's Trove, liquidating her 5 ether and 900LAI.
    ...
    it("withdrawColl(): reverts when calling address does not have active trove", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
    ...
    it("withdrawColl(): reverts if system is in Recovery Mode", async () => {
      ...
      // price drops to 1ETH:150LAI, reducing TCR below 150%
    ...
    it("withdrawColl(): applies pending rewards and updates user's L_ETH, L_LAIDebt snapshots", async () => {
      // --- SETUP ---
      ...
      // close Carol's Trove, liquidating her 1 ether and 180LAI.
      await troveManager.liquidate(carol, { from: owner });
      ...
      const pendingDebtReward_A = await troveManager.getPendingLAIDebtReward(alice)
      const pendingCollReward_B = await troveManager.getPendingETHReward(bob)
      const pendingDebtReward_B = await troveManager.getPendingLAIDebtReward(bob)
    ...
    it("withdrawLAI(): reverts when withdrawal would leave trove with ICR < MCR", async () => {
      // alice creates a Trove and adds first collateral
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: bob } })
      ...
      const LAIwithdrawal = 1  // withdraw 1 wei LAI

     await assertRevert(borrowerOperations.withdrawLAI(th._100pct, LAIwithdrawal, alice, alice, { from: alice }),
      "BorrowerOps: An operation that would result in ICR < MCR is not permitted")
    })

    it("withdrawLAI(): decays a non-zero base rate", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })

      await openTrove({ extraLAIAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraLAIAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraLAIAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({ extraLAIAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })

      const A_LAIBal = await laiToken.balanceOf(A)
      ...
      // D withdraws LAI
      await borrowerOperations.withdrawLAI(th._100pct, dec(1, 18), A, A, { from: D })
      ...
      // E withdraws LAI
      await borrowerOperations.withdrawLAI(th._100pct, dec(1, 18), A, A, { from: E })

      const baseRate_3 = await troveManager.baseRate()
      assert.isTrue(baseRate_3.lt(baseRate_2))
    })

    it("withdrawLAI(): reverts if max fee > 100%", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(10, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraLAIAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraLAIAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraLAIAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })

      await assertRevert(borrowerOperations.withdrawLAI(dec(2, 18), dec(1, 18), A, A, { from: A }), "Max fee percentage must be between 0.5% and 100%")
      await assertRevert(borrowerOperations.withdrawLAI('1000000000000000001', dec(1, 18), A, A, { from: A }), "Max fee percentage must be between 0.5% and 100%")
    })

    it("withdrawLAI(): reverts if max fee < 0.5% in Normal mode", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(10, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraLAIAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraLAIAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraLAIAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })

      await assertRevert(borrowerOperations.withdrawLAI(0, dec(1, 18), A, A, { from: A }), "Max fee percentage must be between 0.5% and 100%")
      await assertRevert(borrowerOperations.withdrawLAI(1, dec(1, 18), A, A, { from: A }), "Max fee percentage must be between 0.5% and 100%")
      await assertRevert(borrowerOperations.withdrawLAI('4999999999999999', dec(1, 18), A, A, { from: A }), "Max fee percentage must be between 0.5% and 100%")
    })

    it("withdrawLAI(): reverts if fee exceeds max fee percentage", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(60, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraLAIAmount: toBN(dec(60, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraLAIAmount: toBN(dec(70, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraLAIAmount: toBN(dec(80, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({ extraLAIAmount: toBN(dec(180, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })

      const totalSupply = await laiToken.totalSupply()
      ...
      // LAIFee:                  15000000558793542
      // absolute _fee:            15000000558793542
      // actual feePercentage:      5000000186264514
      // user's _maxFeePercentage: 49999999999999999

      const lessThan5pct = '49999999999999999'
      await assertRevert(borrowerOperations.withdrawLAI(lessThan5pct, dec(3, 18), A, A, { from: A }), "Fee exceeded provided maximum")

      baseRate = await troveManager.baseRate() // expect 5% base rate
      assert.equal(baseRate, dec(5, 16))
      // Attempt with maxFee 1%
      await assertRevert(borrowerOperations.withdrawLAI(dec(1, 16), dec(1, 18), A, A, { from: B }), "Fee exceeded provided maximum")

      baseRate = await troveManager.baseRate()  // expect 5% base rate
      assert.equal(baseRate, dec(5, 16))
      // Attempt with maxFee 3.754%
      await assertRevert(borrowerOperations.withdrawLAI(dec(3754, 13), dec(1, 18), A, A, { from: C }), "Fee exceeded provided maximum")

      baseRate = await troveManager.baseRate()  // expect 5% base rate
      assert.equal(baseRate, dec(5, 16))
      // Attempt with maxFee 0.5%%
      await assertRevert(borrowerOperations.withdrawLAI(dec(5, 15), dec(1, 18), A, A, { from: D }), "Fee exceeded provided maximum")
    })

    it("withdrawLAI(): succeeds when fee is less than max fee percentage", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(60, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraLAIAmount: toBN(dec(60, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraLAIAmount: toBN(dec(70, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraLAIAmount: toBN(dec(80, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      ...
      const tx1 = await borrowerOperations.withdrawLAI(moreThan5pct, dec(1, 18), A, A, { from: A })
      ...
      // Attempt with maxFee = 5%
      const tx2 = await borrowerOperations.withdrawLAI(dec(5, 16), dec(1, 18), A, A, { from: B })
      ...
      const tx3 = await borrowerOperations.withdrawLAI(dec(1, 17), dec(1, 18), A, A, { from: C })
      ...
      const tx4 = await borrowerOperations.withdrawLAI(dec(37659, 13), dec(1, 18), A, A, { from: D })
      assert.isTrue(tx4.receipt.status)

      // Attempt with maxFee 100%
      const tx5 = await borrowerOperations.withdrawLAI(dec(1, 18), dec(1, 18), A, A, { from: E })
      assert.isTrue(tx5.receipt.status)
    })

    it("withdrawLAI(): doesn't change base rate if it is already zero", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })

      await openTrove({ extraLAIAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraLAIAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraLAIAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraLAIAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({ extraLAIAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      ...
      // D withdraws LAI
      await borrowerOperations.withdrawLAI(th._100pct, dec(37, 18), A, A, { from: D })
      ...
      // E opens trove
      await borrowerOperations.withdrawLAI(th._100pct, dec(12, 18), A, A, { from: E })

      const baseRate_3 = await troveManager.baseRate()
      assert.equal(baseRate_3, '0')
    })

    it("withdrawLAI(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      await borrowerOperations.withdrawLAI(th._100pct, dec(1, 18), C, C, { from: C })
    ...
    it("withdrawLAI(): borrower can't grief the baseRate and stop it decaying by issuing debt at higher frequency than the decay granularity", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ extraLAIAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraLAIAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraLAIAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
    ...
    it("withdrawLAI(): borrowing at non-zero base rate sends LAI fee to LAO staking contract", async () => {
      // time fast-forwards 1 year, and multisig stakes 1 LAO
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
      await laoToken.approve(laoStaking.address, dec(1, 18), { from: multisig })
      await laoStaking.stake(dec(1, 18), { from: multisig })

      // Check LAO LAI balance before == 0
      const laoStaking_LAIBalance_Before = await laiToken.balanceOf(laoStaking.address)
      assert.equal(laoStaking_LAIBalance_Before, '0')
      ...
      // D withdraws LAI
      await borrowerOperations.withdrawLAI(th._100pct, dec(37, 18), C, C, { from: D })

      // Check LAO LAI balance after has increased
      const laoStaking_LAIBalance_After = await laiToken.balanceOf(laoStaking.address)
      assert.isTrue(laoStaking_LAIBalance_After.gt(laoStaking_LAIBalance_Before))
    })

    if (!withProxy) { // TODO: use rawLogs instead of logs
      it("withdrawLAI(): borrowing at non-zero base records the (drawn debt + fee) on the Trove struct", async () => {
        // time fast-forwards 1 year, and multisig stakes 1 LAO
        await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
        await laoToken.approve(laoStaking.address, dec(1, 18), { from: multisig })
        await laoStaking.stake(dec(1, 18), { from: multisig })
        ...
        const withdrawalTx = await borrowerOperations.withdrawLAI(th._100pct, toBN(dec(37, 18)), D, D, { from: D })

        const emittedFee = toBN(th.getLAIFeeFromLAIBorrowingEvent(withdrawalTx))
    ...
    it("withdrawLAI(): Borrowing at non-zero base rate increases the LAO staking contract LAI fees-per-unit-staked", async () => {
      // time fast-forwards 1 year, and multisig stakes 1 LAO
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
      await laoToken.approve(laoStaking.address, dec(1, 18), { from: multisig })
      await laoStaking.stake(dec(1, 18), { from: multisig })

      // Check LAO contract LAI fees-per-unit-staked is zero
      const F_LAI_Before = await laoStaking.F_LAI()
      ...
      // D withdraws LAI
      await borrowerOperations.withdrawLAI(th._100pct, toBN(dec(37, 18)), D, D, { from: D })

      // Check LAO contract LAI fees-per-unit-staked has increased
      const F_LAI_After = await laoStaking.F_LAI()
      assert.isTrue(F_LAI_After.gt(F_LAI_Before))
    })

    it("withdrawLAI(): Borrowing at non-zero base rate sends requested amount to the user", async () => {
      // time fast-forwards 1 year, and multisig stakes 1 LAO
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
      await laoToken.approve(laoStaking.address, dec(1, 18), { from: multisig })
      await laoStaking.stake(dec(1, 18), { from: multisig })

      // Check LAO Staking contract balance before == 0
      const laoStaking_LAIBalance_Before = await laiToken.balanceOf(laoStaking.address)
      assert.equal(laoStaking_LAIBalance_Before, '0')
      ...
      const D_LAIBalanceBefore = await laiToken.balanceOf(D)

      // D withdraws LAI
      const D_LAIRequest = toBN(dec(37, 18))
      await borrowerOperations.withdrawLAI(th._100pct, D_LAIRequest, D, D, { from: D })

      // Check LAO staking LAI balance has increased
      const laoStaking_LAIBalance_After = await laiToken.balanceOf(laoStaking.address)
      assert.isTrue(laoStaking_LAIBalance_After.gt(laoStaking_LAIBalance_Before))

      // Check D's LAI balance now equals their initial balance plus request LAI
      const D_LAIBalanceAfter = await laiToken.balanceOf(D)
      assert.isTrue(D_LAIBalanceAfter.eq(D_LAIBalanceBefore.add(D_LAIRequest)))
    })

    it("withdrawLAI(): Borrowing at zero base rate changes LAI fees-per-unit-staked", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ extraLAIAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraLAIAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraLAIAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      ...
      // A artificially receives LAO, then stakes it
      await laoToken.unprotectedMint(A, dec(100, 18))
      ...
      // D withdraws LAI
      await borrowerOperations.withdrawLAI(th._100pct, dec(37, 18), D, D, { from: D })

      // Check LAO LAI balance after > 0
      const F_LAI_After = await laoStaking.F_LAI()
      assert.isTrue(F_LAI_After.gt('0'))
    })

    it("withdrawLAI(): Borrowing at zero base rate sends debt request to user", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ extraLAIAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraLAIAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraLAIAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      ...
      // Check D's LAI balance now equals their requested LAI
      const D_LAIBalanceAfter = await laiToken.balanceOf(D)

      // Check D's trove debt == D's LAI balance + liquidation reserve
      assert.isTrue(D_LAIBalanceAfter.eq(D_LAIBalanceBefore.add(D_LAIRequest)))
    })

    it("withdrawLAI(): reverts when calling address does not have active trove", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: bob } })

      // Bob successfully withdraws LAI
      const txBob = await borrowerOperations.withdrawLAI(th._100pct, dec(100, 18), bob, bob, { from: bob })
      assert.isTrue(txBob.receipt.status)

      // Carol with no active trove attempts to withdraw LAI
      try {
        const txCarol = await borrowerOperations.withdrawLAI(th._100pct, dec(100, 18), carol, carol, { from: carol })
        assert.isFalse(txCarol.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    it("withdrawLAI(): reverts when requested withdrawal amount is zero LAI", async () => {
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: bob } })

      // Bob successfully withdraws 1e-18 LAI
      const txBob = await borrowerOperations.withdrawLAI(th._100pct, 1, bob, bob, { from: bob })
      assert.isTrue(txBob.receipt.status)

      // Alice attempts to withdraw 0 LAI
      try {
        const txAlice = await borrowerOperations.withdrawLAI(th._100pct, 0, alice, alice, { from: alice })
    ...
    it("withdrawLAI(): reverts when system is in Recovery Mode", async () => {
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: carol } })
      ...
      // Withdrawal possible when recoveryMode == false
      const txAlice = await borrowerOperations.withdrawLAI(th._100pct, dec(100, 18), alice, alice, { from: alice })
      assert.isTrue(txAlice.receipt.status)
      ...
      //Check LAI withdrawal impossible when recoveryMode == true
      try {
        const txBob = await borrowerOperations.withdrawLAI(th._100pct, 1, bob, bob, { from: bob })
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
    ...
    it("withdrawLAI(): reverts when withdrawal would bring the trove's ICR < MCR", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(11, 17)), extraParams: { from: bob } })

      // Bob tries to withdraw LAI that would bring his ICR < MCR
      try {
        const txBob = await borrowerOperations.withdrawLAI(th._100pct, 1, bob, bob, { from: bob })
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
    ...
    it("withdrawLAI(): reverts when a withdrawal would cause the TCR of the system to fall below the CCR", async () => {
      await priceFeed.setPrice(dec(100, 18))
      const price = await priceFeed.getPrice()
      ...
      // Bob attempts to withdraw 1 LAI.
      // System TCR would be: ((3+3) * 100 ) / (200+201) = 600/401 = 149.62%, i.e. below CCR of 150%.
      try {
        const txBob = await borrowerOperations.withdrawLAI(th._100pct, dec(1, 18), bob, bob, { from: bob })
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    it("withdrawLAI(): reverts if system is in Recovery Mode", async () => {
      // --- SETUP ---
      await openTrove({ ICR: toBN(dec(15, 17)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(15, 17)), extraParams: { from: bob } })
      ...
      try {
        const txData = await borrowerOperations.withdrawLAI(th._100pct, '200', alice, alice, { from: alice })
        assert.isFalse(txData.receipt.status)
      } catch (err) {
        assert.include(err.message, 'revert')
      }
    ...
    it("withdrawLAI(): increases the Trove's LAI debt by the correct amount", async () => {
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      ...
      await borrowerOperations.withdrawLAI(th._100pct, await getNetBorrowingAmount(100), alice, alice, { from: alice })
    ...
    it("withdrawLAI(): increases LAI debt in ActivePool by correct amount", async () => {
      ...
      // check before
      const activePool_LAI_Before = await activePool.getLAIDebt()
      assert.isTrue(activePool_LAI_Before.eq(aliceDebtBefore))

      await borrowerOperations.withdrawLAI(th._100pct, await getNetBorrowingAmount(dec(10000, 18)), alice, alice, { from: alice })

      // check after
      const activePool_LAI_After = await activePool.getLAIDebt()
      th.assertIsApproximatelyEqual(activePool_LAI_After, activePool_LAI_Before.add(toBN(dec(10000, 18))))
    })

    it("withdrawLAI(): increases user LAIToken balance by correct amount", async () => {
      await openTrove({ extraParams: { value: toBN(dec(100, 'ether')), from: alice } })

      // check before
      const alice_LAITokenBalance_Before = await laiToken.balanceOf(alice)
      assert.isTrue(alice_LAITokenBalance_Before.gt(toBN('0')))

      await borrowerOperations.withdrawLAI(th._100pct, dec(10000, 18), alice, alice, { from: alice })

      // check after
      const alice_LAITokenBalance_After = await laiToken.balanceOf(alice)
      assert.isTrue(alice_LAITokenBalance_After.eq(alice_LAITokenBalance_Before.add(toBN(dec(10000, 18)))))
    })

    // --- repayLAI() ---
    it("repayLAI(): reverts when repayment would leave trove with ICR < MCR", async () => {
      // alice creates a Trove and adds first collateral
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: bob } })
      ...
      const LAIRepayment = 1  // 1 wei repayment

     await assertRevert(borrowerOperations.repayLAI(LAIRepayment, alice, alice, { from: alice }),
      "BorrowerOps: An operation that would result in ICR < MCR is not permitted")
    })

    it("repayLAI(): Succeeds when it would leave trove with net debt >= minimum net debt", async () => {
      // Make the LAI request 2 wei above min net debt to correct for floor division, and make net debt = min net debt + 1 wei
      ...
      const repayTxA = await borrowerOperations.repayLAI(1, A, A, { from: A })
      assert.isTrue(repayTxA.receipt.status)

      await borrowerOperations.openTrove(th._100pct, dec(20, 25), B, B, { from: B, value: dec(100, 30) })

      const repayTxB = await borrowerOperations.repayLAI(dec(19, 25), B, B, { from: B })
      assert.isTrue(repayTxB.receipt.status)
    })

    it("repayLAI(): reverts when it would leave trove with net debt < minimum net debt", async () => {
      // Make the LAI request 2 wei above min net debt to correct for floor division, and make net debt = min net debt + 1 wei
      await borrowerOperations.openTrove(th._100pct, await getNetBorrowingAmount(MIN_NET_DEBT.add(toBN('2'))), A, A, { from: A, value: dec(100, 30) })

      const repayTxAPromise = borrowerOperations.repayLAI(2, A, A, { from: A })
      await assertRevert(repayTxAPromise, "BorrowerOps: Trove's net debt must be greater than minimum")
    })

    it("repayLAI(): reverts when calling address does not have active trove", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      // Bob successfully repays some LAI
      const txBob = await borrowerOperations.repayLAI(dec(10, 18), bob, bob, { from: bob })
      assert.isTrue(txBob.receipt.status)

      // Carol with no active trove attempts to repayLAI
      try {
        const txCarol = await borrowerOperations.repayLAI(dec(10, 18), carol, carol, { from: carol })
        assert.isFalse(txCarol.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    it("repayLAI(): reverts when attempted repayment is > the debt of the trove", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      ...
      try {
        const txAlice = await borrowerOperations.repayLAI(aliceDebt.add(toBN(dec(1, 18))), alice, alice, { from: alice })
        assert.isFalse(txAlice.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    //repayLAI: reduces LAI debt in Trove
    it("repayLAI(): reduces the Trove's LAI debt by the correct amount", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      const aliceDebtBefore = await getTroveEntireDebt(alice)
      assert.isTrue(aliceDebtBefore.gt(toBN('0')))

      await borrowerOperations.repayLAI(aliceDebtBefore.div(toBN(10)), alice, alice, { from: alice })  // Repays 1/10 her debt
    ...
    it("repayLAI(): decreases LAI debt in ActivePool by correct amount", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      const aliceDebtBefore = await getTroveEntireDebt(alice)
      assert.isTrue(aliceDebtBefore.gt(toBN('0')))
      ...
      // Check before
      const activePool_LAI_Before = await activePool.getLAIDebt()
      assert.isTrue(activePool_LAI_Before.gt(toBN('0')))

      await borrowerOperations.repayLAI(aliceDebtBefore.div(toBN(10)), alice, alice, { from: alice })  // Repays 1/10 her debt

      // check after
      const activePool_LAI_After = await activePool.getLAIDebt()
      th.assertIsApproximatelyEqual(activePool_LAI_After, activePool_LAI_Before.sub(aliceDebtBefore.div(toBN(10))))
    })

    it("repayLAI(): decreases user LAIToken balance by correct amount", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      const aliceDebtBefore = await getTroveEntireDebt(alice)
      assert.isTrue(aliceDebtBefore.gt(toBN('0')))
      ...
      // check after
      const alice_LAITokenBalance_After = await laiToken.balanceOf(alice)
      th.assertIsApproximatelyEqual(alice_LAITokenBalance_After, alice_LAITokenBalance_Before.sub(aliceDebtBefore.div(toBN(10))))
    })

    it('repayLAI(): can repay debt in Recovery Mode', async () => {
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      const aliceDebtBefore = await getTroveEntireDebt(alice)
      assert.isTrue(aliceDebtBefore.gt(toBN('0')))
      ...
      const tx = await borrowerOperations.repayLAI(aliceDebtBefore.div(toBN(10)), alice, alice, { from: alice })
    ...
    it("repayLAI(): Reverts if borrower has insufficient LAI balance to cover his debt repayment", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      const bobBalBefore = await laiToken.balanceOf(B)
      assert.isTrue(bobBalBefore.gt(toBN('0')))

      // Bob transfers all but 5 of his LAI to Carol
      await laiToken.transfer(C, bobBalBefore.sub((toBN(dec(5, 18)))), { from: B })

      //Confirm B's LAI balance has decreased to 5 LAI
      const bobBalAfter = await laiToken.balanceOf(B)

      assert.isTrue(bobBalAfter.eq(toBN(dec(5, 18))))

      // Bob tries to repay 6 LAI
      const repayLAIPromise_B = borrowerOperations.repayLAI(toBN(dec(6, 18)), B, B, { from: B })

      await assertRevert(repayLAIPromise_B, "Caller doesnt have enough LAI to make repayment")
    })
    ...
    it("adjustTrove(): reverts when adjustment would leave trove with ICR < MCR", async () => {
     ...
     await assertRevert(borrowerOperations.adjustTrove(th._100pct, 0, LAIRepayment, false, alice, alice, { from: alice, value: collTopUp }),
      "BorrowerOps: An operation that would result in ICR < MCR is not permitted")
    })

    it("adjustTrove(): reverts if max fee < 0.5% in Normal mode", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
    ...
    it("adjustTrove(): doesn't change base rate if it is already zero", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      await openTrove({ extraLAIAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
    ...
    it("adjustTrove(): borrowing at non-zero base rate sends LAI fee to LAO staking contract", async () => {
      // time fast-forwards 1 year, and multisig stakes 1 LAO
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
      await laoToken.approve(laoStaking.address, dec(1, 18), { from: multisig })
      await laoStaking.stake(dec(1, 18), { from: multisig })
      ...
      // D adjusts trove
      await openTrove({ extraLAIAmount: toBN(dec(37, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
    ...
    if (!withProxy) { // TODO: use rawLogs instead of logs
      it("adjustTrove(): borrowing at non-zero base records the (drawn debt + fee) on the Trove struct", async () => {
        ...
        const emittedFee = toBN(th.getLAIFeeFromLAIBorrowingEvent(adjustmentTx))
        assert.isTrue(emittedFee.gt(toBN('0')))
    ...
    it("adjustTrove(): Borrowing at non-zero base rate increases the LAO staking contract LAI fees-per-unit-staked", async () => {
      // time fast-forwards 1 year, and multisig stakes 1 LAO
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
      await laoToken.approve(laoStaking.address, dec(1, 18), { from: multisig })
      await laoStaking.stake(dec(1, 18), { from: multisig })
    ...
    it("adjustTrove(): Borrowing at non-zero base rate sends requested amount to the user", async () => {
      ...
      // D adjusts trove
      const LAIRequest_D = toBN(dec(40, 18))
      await borrowerOperations.adjustTrove(th._100pct, 0, LAIRequest_D, true, D, D, { from: D })

      // Check LAO staking LAI balance has increased
      const laoStaking_LAIBalance_After = await laiToken.balanceOf(laoStaking.address)
      assert.isTrue(laoStaking_LAIBalance_After.gt(laoStaking_LAIBalance_Before))

      // Check D's LAI balance has increased by their requested LAI
      const D_LAIBalanceAfter = await laiToken.balanceOf(D)
      assert.isTrue(D_LAIBalanceAfter.eq(D_LAIBalanceBefore.add(LAIRequest_D)))
    })

    it("adjustTrove(): Borrowing at zero base rate changes LAI balance of LAO staking contract", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ extraLAIAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraLAIAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraLAIAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      ...
      // Check staking LAI balance before > 0
      const laoStaking_LAIBalance_Before = await laiToken.balanceOf(laoStaking.address)
      assert.isTrue(laoStaking_LAIBalance_Before.gt(toBN('0')))

      // D adjusts trove
      await borrowerOperations.adjustTrove(th._100pct, 0, dec(37, 18), true, D, D, { from: D })

      // Check staking LAI balance after > staking balance before
      const laoStaking_LAIBalance_After = await laiToken.balanceOf(laoStaking.address)
      assert.isTrue(laoStaking_LAIBalance_After.gt(laoStaking_LAIBalance_Before))
    })

    it("adjustTrove(): Borrowing at zero base rate changes LAO staking contract LAI fees-per-unit-staked", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(20000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale, value: toBN(dec(100, 'ether')) } })
      await openTrove({ extraLAIAmount: toBN(dec(40000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraLAIAmount: toBN(dec(40000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraLAIAmount: toBN(dec(40000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraLAIAmount: toBN(dec(40000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      ...
      // Check staking LAI balance before == 0
      const F_LAI_Before = await laoStaking.F_LAI()
      assert.isTrue(F_LAI_Before.eq(toBN('0')))

      // D adjusts trove
      await borrowerOperations.adjustTrove(th._100pct, 0, dec(37, 18), true, D, D, { from: D })

      // Check staking LAI balance increases
      const F_LAI_After = await laoStaking.F_LAI()
      assert.isTrue(F_LAI_After.gt(F_LAI_Before))
    })

    it("adjustTrove(): Borrowing at zero base rate sends total requested LAI to the user", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(20000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale, value: toBN(dec(100, 'ether')) } })
      await openTrove({ extraLAIAmount: toBN(dec(40000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraLAIAmount: toBN(dec(40000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraLAIAmount: toBN(dec(40000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      ...
      const D_LAIBalBefore = await laiToken.balanceOf(D)
      // Check baseRate is zero
      const baseRate_1 = await troveManager.baseRate()
      assert.equal(baseRate_1, '0')
      ...
      const DUSDBalanceBefore = await laiToken.balanceOf(D)

      // D adjusts trove
      const LAIRequest_D = toBN(dec(40, 18))
      await borrowerOperations.adjustTrove(th._100pct, 0, LAIRequest_D, true, D, D, { from: D })

      // Check D's LAI balance increased by their requested LAI
      const LAIBalanceAfter = await laiToken.balanceOf(D)
      assert.isTrue(LAIBalanceAfter.eq(D_LAIBalBefore.add(LAIRequest_D)))
    })

    it("adjustTrove(): reverts when calling address has no active trove", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraLAIAmount: toBN(dec(20000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })

      // Alice coll and debt increase(+1 ETH, +50LAI)
    ...
    it("adjustTrove(): debt increase that would reduce the ICR reverts in Recovery Mode", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(3, 18)), extraParams: { from: alice } })
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      const CCR = await troveManager.CCR()
    ...
    it("adjustTrove(): debt increase in Recovery Mode charges no fee", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraLAIAmount: toBN(dec(200000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      ...
      // B stakes LAO
      await laoToken.unprotectedMint(bob, dec(100, 18))
      await laoStaking.stake(dec(100, 18), { from: bob })

      const laoStakingLAIBalanceBefore = await laiToken.balanceOf(laoStaking.address)
      assert.isTrue(laoStakingLAIBalanceBefore.gt(toBN('0')))
      ...
      // Check emitted fee = 0
      const emittedFee = toBN(await th.getEventArgByName(txAlice, 'LAIBorrowingFeePaid', '_LAIFee'))
      assert.isTrue(emittedFee.eq(toBN('0')))

      assert.isTrue(await th.checkRecoveryMode(contracts))

      // Check no fee was sent to staking contract
      const laoStakingLAIBalanceAfter = await laiToken.balanceOf(laoStaking.address)
      assert.equal(laoStakingLAIBalanceAfter.toString(), laoStakingLAIBalanceBefore.toString())
    })
    ...
    it("adjustTrove(): reverts when LAI repaid is > debt of the trove", async () => {
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      ...
      const bobFee = toBN(await th.getEventArgByIndex(bobOpenTx, 'LAIBorrowingFeePaid', 1))
      assert.isTrue(bobFee.gt(toBN('0')))

      // Alice transfers LAI to bob to compensate borrowing fees
      await laiToken.transfer(bob, bobFee, { from: alice })

      const remainingDebt = (await troveManager.getTroveDebt(bob)).sub(LAI_GAS_COMPENSATION)
    ...
    it("adjustTrove(): reverts when change would cause the ICR of the trove to fall below the MCR", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(100, 18)), extraParams: { from: whale } })

      await priceFeed.setPrice(dec(100, 18))

      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(11, 17)), extraParams: { from: alice } })
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(11, 17)), extraParams: { from: bob } })

      // Bob attempts to increase debt by 100 LAI and 1 ether, i.e. a change that constitutes a 100% ratio of coll:debt.
    ...
    it("adjustTrove(): With 0 coll change, doesnt change borrower's coll or ActivePool coll", async () => {
      ...
      // Alice adjusts trove. No coll change, and a debt increase (+50LAI)
    ...
    it("adjustTrove(): With 0 debt change, doesnt change borrower's debt or ActivePool debt", async () => {
      ...
      const aliceDebtBefore = await getTroveEntireDebt(alice)
      const activePoolDebtBefore = await activePool.getLAIDebt()
      ...
      const aliceDebtAfter = await getTroveEntireDebt(alice)
      const activePoolDebtAfter = await activePool.getLAIDebt()
    ...
    it("adjustTrove(): updates borrower's debt and coll with an increase in both", async () => {
      ...
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: alice } })
      ...
      // Alice adjusts trove. Coll and debt increase(+1 ETH, +50LAI)
    ...
    it("adjustTrove(): updates borrower's debt and coll with a decrease in both", async () => {
      ...
      // Alice adjusts trove coll and debt decrease (-0.5 ETH, -50LAI)
    ...
    it("adjustTrove(): updates borrower's  debt and coll with coll increase, debt decrease", async () => {
      ...
      // Alice adjusts trove - coll increase and debt decrease (+0.5 ETH, -50LAI)
    ...
    it("adjustTrove(): updates borrower's debt and coll with coll decrease, debt increase", async () => {
      ...
      // Alice adjusts trove - coll decrease and debt increase (0.1 ETH, 10LAI)
    ...
    it("adjustTrove(): updates borrower's stake and totalStakes with a coll increase", async () => {
      ...
      // Alice adjusts trove - coll and debt increase (+1 ETH, +50 LAI)
      await borrowerOperations.adjustTrove(th._100pct, 0, dec(50, 18), true, alice, alice, { from: alice, value: dec(1, 'ether') })
    ...
    it("adjustTrove(): changes LAIToken balance by the requested decrease", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      // check after
      const alice_LAITokenBalance_After = await laiToken.balanceOf(alice)
      assert.isTrue(alice_LAITokenBalance_After.eq(alice_LAITokenBalance_Before.sub(toBN(dec(10, 18)))))
    })

    it("adjustTrove(): changes LAIToken balance by the requested increase", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      // check after
      const alice_LAITokenBalance_After = await laiToken.balanceOf(alice)
      assert.isTrue(alice_LAITokenBalance_After.eq(alice_LAITokenBalance_Before.add(toBN(dec(100, 18)))))
    })
    ...
    it("adjustTrove(): Changes the LAI debt in ActivePool by requested decrease", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })

      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: alice } })

      const activePool_LAIDebt_Before = await activePool.getLAIDebt()
      assert.isTrue(activePool_LAIDebt_Before.gt(toBN('0')))
      ...
      const activePool_LAIDebt_After = await activePool.getLAIDebt()
      assert.isTrue(activePool_LAIDebt_After.eq(activePool_LAIDebt_Before.sub(toBN(dec(30, 18)))))
    })

    it("adjustTrove(): Changes the LAI debt in ActivePool by requested increase", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: alice } })
      ...
      th.assertIsApproximatelyEqual(activePool_LAIDebt_After, activePool_LAIDebt_Before.add(toBN(dec(100, 18))))
    })
    ...
    it("adjustTrove(): Reverts if borrower has insufficient LAI balance to cover his debt repayment", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: B } })
      const bobDebt = await getTroveEntireDebt(B)

      // Bob transfers some LAI to carol
      await laiToken.transfer(C, dec(10, 18), { from: B })

      //Confirm B's LAI balance is less than 50 LAI
      const B_LAIBal = await laiToken.balanceOf(B)
      assert.isTrue(B_LAIBal.lt(bobDebt))

      const repayLAIPromise_B = borrowerOperations.adjustTrove(th._100pct, 0, bobDebt, false, B, B, { from: B })

      // B attempts to repay all his debt
      await assertRevert(repayLAIPromise_B, "revert")
    })
    ...
    if (!withProxy) { // no need to test this with proxies
      it("Internal _adjustTrove(): reverts when op is a withdrawal and _borrower param is not the msg.sender", async () => {
        await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
        await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: bob } })
    ...
    it("closeTrove(): reverts when it would lower the TCR below CCR", async () => {
      ...
      await openTrove({ ICR: toBN(dec(120, 16)), extraLAIAmount: toBN(dec(300, 18)), extraParams:{ from: bob } })

      const price = await priceFeed.getPrice()

      // to compensate borrowing fees
      await laiToken.transfer(alice, dec(300, 18), { from: bob })
    ...
    it("closeTrove(): reverts when system is in Recovery Mode", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(100000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: carol } })

      // Alice transfers her LAI to Bob and Carol so they can cover fees
      const aliceBal = await laiToken.balanceOf(alice)
      await laiToken.transfer(bob, aliceBal.div(toBN(2)), { from: alice })
      await laiToken.transfer(carol, aliceBal.div(toBN(2)), { from: alice })
    ...
    it("closeTrove(): reverts when trove is the only one in the system", async () => {
      ...
      await laiToken.unprotectedMint(alice, dec(100000, 18))

      // Check she has more LAI than her trove debt
      const aliceBal = await laiToken.balanceOf(alice)
      const aliceDebt = await getTroveEntireDebt(alice)
    ...
    it("closeTrove(): reduces a Trove's collateral to zero", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: dennis } })

      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })

      const aliceCollBefore = await getTroveEntireColl(alice)
      const dennisLAI = await laiToken.balanceOf(dennis)
      assert.isTrue(aliceCollBefore.gt(toBN('0')))
      assert.isTrue(dennisLAI.gt(toBN('0')))

      // To compensate borrowing fees
      await laiToken.transfer(alice, dennisLAI.div(toBN(2)), { from: dennis })
    ...
    it("closeTrove(): zero's the troves reward snapshots", async () => {
      ...
      const L_LAIDebt_A_Snapshot = (await troveManager.rewardSnapshots(alice))[1]
      assert.isTrue(L_ETH_A_Snapshot.gt(toBN('0')))
      assert.isTrue(L_LAIDebt_A_Snapshot.gt(toBN('0')))
      ...
      const L_LAIDebt_Snapshot_A_AfterLiquidation = (await troveManager.rewardSnapshots(alice))[1]

      assert.isTrue(L_ETH_Snapshot_A_AfterLiquidation.gt(toBN('0')))
      assert.isTrue(L_LAIDebt_Snapshot_A_AfterLiquidation.gt(toBN('0')))

      // to compensate borrowing fees
      await laiToken.transfer(alice, await laiToken.balanceOf(dennis), { from: dennis })
      ...
      const L_LAIDebt_Snapshot_A_afterAliceCloses = (await troveManager.rewardSnapshots(alice))[1]

      assert.equal(L_ETH_Snapshot_A_afterAliceCloses, '0')
      assert.equal(L_LAIDebt_Snapshot_A_afterAliceCloses, '0')
    })
    ...
    it("closeTrove(): reduces ActivePool debt by correct amount", async () => {
      ...
      // Check before
      const activePool_Debt_before = await activePool.getLAIDebt()
      ...
      // Check after
      const activePool_Debt_After = (await activePool.getLAIDebt()).toString()
    ...
    it("closeTrove(): subtracts the debt of the closed Trove from the Borrower's LAIToken balance", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: dennis } })
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      ...
      const alice_LAIBalance_Before = await laiToken.balanceOf(alice)
      assert.isTrue(alice_LAIBalance_Before.gt(toBN('0')))

      // close trove
      await borrowerOperations.closeTrove({ from: alice })

      // check alice LAI balance after
      const alice_LAIBalance_After = await laiToken.balanceOf(alice)
      th.assertIsApproximatelyEqual(alice_LAIBalance_After, alice_LAIBalance_Before.sub(aliceDebt.sub(LAI_GAS_COMPENSATION)))
    })

    it("closeTrove(): applies pending rewards", async () => {
      // --- SETUP ---
      await openTrove({ extraLAIAmount: toBN(dec(1000000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      const whaleDebt = await getTroveEntireDebt(whale)
      const whaleColl = await getTroveEntireColl(whale)

      await openTrove({ extraLAIAmount: toBN(dec(15000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraLAIAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: carol } })
      ...
      await laiToken.transfer(alice, dec(10000, 18), { from: whale })
      await laiToken.transfer(bob, dec(10000, 18), { from: whale })
      ...
      const defaultPool_LAIDebt = await defaultPool.getLAIDebt()

      // Carol's liquidated coll (1 ETH) and drawn debt should have entered the Default Pool
      assert.isAtMost(th.getDifference(defaultPool_ETH, liquidatedColl_C), 100)
      assert.isAtMost(th.getDifference(defaultPool_LAIDebt, liquidatedDebt_C), 100)
      ...
      const defaultPool_LAIDebt_afterAliceCloses = await defaultPool.getLAIDebt()

      assert.isAtMost(th.getDifference(defaultPool_ETH_afterAliceCloses,
        defaultPool_ETH.sub(pendingCollReward_A)), 1000)
      assert.isAtMost(th.getDifference(defaultPool_LAIDebt_afterAliceCloses,
        defaultPool_LAIDebt.sub(pendingDebtReward_A)), 1000)
      ...
      const defaultPool_LAIDebt_afterBobCloses = await defaultPool.getLAIDebt()

      assert.isAtMost(th.getDifference(defaultPool_ETH_afterBobCloses, 0), 100000)
      assert.isAtMost(th.getDifference(defaultPool_LAIDebt_afterBobCloses, 0), 100000)
    })

    it("closeTrove(): reverts if borrower has insufficient LAI balance to repay his entire debt", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(15000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraLAIAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })

      //Confirm Bob's LAI balance is less than his trove debt
      const B_LAIBal = await laiToken.balanceOf(B)
      const B_troveDebt = await getTroveEntireDebt(B)

      assert.isTrue(B_LAIBal.lt(B_troveDebt))

      const closeTrovePromise_B = borrowerOperations.closeTrove({ from: B })

      // Check closing trove reverts
      await assertRevert(closeTrovePromise_B, "BorrowerOps: Caller doesnt have enough LAI to make repayment")
    })
    ...
    if (!withProxy) { // TODO: use rawLogs instead of logs
      it("openTrove(): emits a TroveUpdated event with the correct collateral and debt", async () => {
        const txA = (await openTrove({ extraLAIAmount: toBN(dec(15000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })).tx
        const txB = (await openTrove({ extraLAIAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })).tx
        const txC = (await openTrove({ extraLAIAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })).tx
        ...
        const txD = (await openTrove({ extraLAIAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })).tx
        const txE = (await openTrove({ extraLAIAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })).tx
    ...
    it("openTrove(): decays a non-zero base rate", async () => {
      ...
      // E opens trove
      await openTrove({ extraLAIAmount: toBN(dec(12, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
    ...
    it("openTrove(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation", async () => {
      ...
      await openTrove({ extraLAIAmount: toBN(dec(1, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      ...
      await openTrove({ extraLAIAmount: toBN(dec(1, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
    ...
    it("openTrove(): borrowing at non-zero base rate sends LAI fee to LAO staking contract", async () => {
      // time fast-forwards 1 year, and multisig stakes 1 LAO
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
      await laoToken.approve(laoStaking.address, dec(1, 18), { from: multisig })
      await laoStaking.stake(dec(1, 18), { from: multisig })
    ...
    if (!withProxy) { // TODO: use rawLogs instead of logs
      it("openTrove(): borrowing at non-zero base records the (drawn debt + fee  + liq. reserve) on the Trove struct", async () => {
        ...
        const D_LAIRequest = toBN(dec(20000, 18))

        // D withdraws LAI
        const openTroveTx = await borrowerOperations.openTrove(th._100pct, D_LAIRequest, ZERO_ADDRESS, ZERO_ADDRESS, { from: D, value: dec(200, 'ether') })

        const emittedFee = toBN(th.getLAIFeeFromLAIBorrowingEvent(openTroveTx))
        assert.isTrue(toBN(emittedFee).gt(toBN('0')))

        const newDebt = (await troveManager.Troves(D))[0]

        // Check debt on Trove struct equals drawn debt plus emitted fee
        th.assertIsApproximatelyEqual(newDebt, D_LAIRequest.add(emittedFee).add(LAI_GAS_COMPENSATION), 100000)
      })
    }

    it("openTrove(): Borrowing at non-zero base rate increases the LAO staking contract LAI fees-per-unit-staked", async () => {
      // time fast-forwards 1 year, and multisig stakes 1 LAO
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
      await laoToken.approve(laoStaking.address, dec(1, 18), { from: multisig })
      await laoStaking.stake(dec(1, 18), { from: multisig })
    ...
    it("openTrove(): Borrowing at non-zero base rate sends requested amount to the user", async () => {
      ...
      // D opens trove
      const LAIRequest_D = toBN(dec(40000, 18))
      await borrowerOperations.openTrove(th._100pct, LAIRequest_D, D, D, { from: D, value: dec(500, 'ether') })
      ...
      // Check D's LAI balance now equals their requested LAI
      const LAIBalance_D = await laiToken.balanceOf(D)
      assert.isTrue(LAIRequest_D.eq(LAIBalance_D))
    })

    it("openTrove(): Borrowing at zero base rate changes the LAO staking contract LAI fees-per-unit-staked", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraLAIAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraLAIAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      ...
      // Check LAI reward per LAO staked == 0
      const F_LAI_Before = await laoStaking.F_LAI()
      assert.equal(F_LAI_Before, '0')
      ...
      // Check LAI reward per LAO staked > 0
      const F_LAI_After = await laoStaking.F_LAI()
      assert.isTrue(F_LAI_After.gt(toBN('0')))
    })
    ...
    it("openTrove(): Borrowing at zero base rate charges minimum fee", async () => {
      ...
      const LAIRequest = toBN(dec(10000, 18))
      const txC = await borrowerOperations.openTrove(th._100pct, LAIRequest, ZERO_ADDRESS, ZERO_ADDRESS, { value: dec(100, 'ether'), from: C })
      const _LAIFee = toBN(th.getEventArgByName(txC, "LAIBorrowingFeePaid", "_LAIFee"))

      const expectedFee = BORROWING_FEE_FLOOR.mul(toBN(LAIRequest)).div(toBN(dec(1, 18)))
      assert.isTrue(_LAIFee.eq(expectedFee))
    })

    it("openTrove(): reverts when system is in Recovery Mode and ICR < CCR", async () => {
      await openTrove({ extraLAIAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale } })
      await openTrove({ extraLAIAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      assert.isFalse(await th.checkRecoveryMode(contracts))
      ...
      try {
        const txBob = await openTrove({ extraLAIAmount: toBN(dec(5000, 18)), ICR: toBN(dec(149, 16)), extraParams: { from: alice } })
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    it("openTrove(): reverts when trove ICR < MCR", async () => {
      ...
      try {
        const txBob = (await openTrove({ extraLAIAmount: toBN(dec(5000, 18)), ICR: toBN(dec(109, 16)), extraParams: { from: bob } })).tx
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
      ...
      try {
        const txBob = await openTrove({ extraLAIAmount: toBN(dec(5000, 18)), ICR: toBN(dec(109, 16)), extraParams: { from: bob } })
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    it("openTrove(): reverts when opening the trove would cause the TCR of the system to fall below the CCR", async () => {
      ...
      await openTrove({ extraLAIAmount: toBN(dec(5000, 18)), ICR: toBN(dec(15, 17)), extraParams: { from: alice } })
      ...
      try {
        const txBob = await openTrove({ extraLAIAmount: toBN(dec(5000, 18)), ICR: toBN(dec(149, 16)), extraParams: { from: bob } })
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    it("openTrove(): reverts if trove is already active", async () => {
      ...
      await openTrove({ extraLAIAmount: toBN(dec(5000, 18)), ICR: toBN(dec(15, 17)), extraParams: { from: alice } })
      await openTrove({ extraLAIAmount: toBN(dec(5000, 18)), ICR: toBN(dec(15, 17)), extraParams: { from: bob } })

      try {
        const txB_1 = await openTrove({ extraLAIAmount: toBN(dec(5000, 18)), ICR: toBN(dec(3, 18)), extraParams: { from: bob } })
    ...
    it("openTrove(): Can open a trove with ICR >= CCR when system is in Recovery Mode", async () => {
      ...
      // price drops to 1ETH:100LAI, reducing TCR below 150%
      await priceFeed.setPrice('100000000000000000000');
      const price = await priceFeed.getPrice()
      ...
      const txCarol = (await openTrove({ extraLAIAmount: toBN(dec(5000, 18)), ICR: toBN(dec(15, 17)), extraParams: { from: carol } })).tx
    ...
    it("openTrove(): creates a new Trove and assigns the correct collateral and debt amount", async () => {
      ...
      const LAIRequest = MIN_NET_DEBT
      borrowerOperations.openTrove(th._100pct, MIN_NET_DEBT, carol, carol, { from: alice, value: dec(100, 'ether') })

      // Get the expected debt based on the LAI request (adding fee and liq. reserve on top)
      const expectedDebt = LAIRequest
        .add(await troveManager.getBorrowingFee(LAIRequest))
        .add(LAI_GAS_COMPENSATION)
    ...
    it("openTrove(): records up-to-date initial snapshots of L_ETH and L_LAIDebt", async () => {
      // --- SETUP ---
      ...
      /* with total stakes = 10 ether, after liquidation, L_ETH should equal 1/10 ether per-ether-staked,
       and L_LAI should equal 18 LAI per-ether-staked. */

      const L_ETH = await troveManager.L_ETH()
      const L_LAI = await troveManager.L_LAIDebt()

      assert.isTrue(L_ETH.gt(toBN('0')))
      assert.isTrue(L_LAI.gt(toBN('0')))

      // Bob opens trove
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })

      // Check Bob's snapshots of L_ETH and L_LAI equal the respective current values
      const bob_rewardSnapshot = await troveManager.rewardSnapshots(bob)
      const bob_ETHrewardSnapshot = bob_rewardSnapshot[0]
      const bob_LAIDebtRewardSnapshot = bob_rewardSnapshot[1]

      assert.isAtMost(th.getDifference(bob_ETHrewardSnapshot, L_ETH), 1000)
      assert.isAtMost(th.getDifference(bob_LAIDebtRewardSnapshot, L_LAI), 1000)
    })

    it("openTrove(): allows a user to open a Trove, then close it, then re-open it", async () => {
      // Open Troves
      await openTrove({ extraLAIAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale } })
      await openTrove({ extraLAIAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraLAIAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: carol } })
    ...
    it("openTrove(): increases the Trove's LAI debt by the correct amount", async () => {
      // check before
      const alice_Trove_Before = await troveManager.Troves(alice)
      const debt_Before = alice_Trove_Before[0]
      assert.equal(debt_Before, 0)

      await borrowerOperations.openTrove(th._100pct, await getOpenTroveLAIAmount(dec(10000, 18)), alice, alice, { from: alice, value: dec(100, 'ether') })
    ...
    it("openTrove(): increases LAI debt in ActivePool by the debt of the trove", async () => {
      const activePool_LAIDebt_Before = await activePool.getLAIDebt()
      assert.equal(activePool_LAIDebt_Before, 0)
      ...
      const activePool_LAIDebt_After = await activePool.getLAIDebt()
      assert.isTrue(activePool_LAIDebt_After.eq(aliceDebt))
    })

    it("openTrove(): increases user LAIToken balance by correct amount", async () => {
      // check before
      const alice_LAITokenBalance_Before = await laiToken.balanceOf(alice)
      assert.equal(alice_LAITokenBalance_Before, 0)

      await borrowerOperations.openTrove(th._100pct, dec(10000, 18), alice, alice, { from: alice, value: dec(100, 'ether') })

      // check after
      const alice_LAITokenBalance_After = await laiToken.balanceOf(alice)
      assert.equal(alice_LAITokenBalance_After, dec(10000, 18))
    })
    ...
    it("getCompositeDebt(): returns debt + gas comp", async () => {
      ...
      assert.equal(res1, LAI_GAS_COMPENSATION.toString())

      const res2 = await borrowerOperations.getCompositeDebt(dec(90, 18))
      th.assertIsApproximatelyEqual(res2, LAI_GAS_COMPENSATION.add(toBN(dec(90, 18))))

      const res3 = await borrowerOperations.getCompositeDebt(dec(24423422357345049, 12))
      th.assertIsApproximatelyEqual(res3, LAI_GAS_COMPENSATION.add(toBN(dec(24423422357345049, 12))))
    })
    ...
    describe("getNewTCRFromTroveChange() returns the correct TCR", async () => {
      ...
      it("collChange = 0, debtChange = 0", async () => {
        ...
        const troveLAIAmount = await getOpenTroveLAIAmount(troveTotalDebt)
        await borrowerOperations.openTrove(th._100pct, troveLAIAmount, alice, alice, { from: alice, value: troveColl })
        await borrowerOperations.openTrove(th._100pct, troveLAIAmount, bob, bob, { from: bob, value: troveColl })
    ...
    if (!withProxy) {
      it('closeTrove(): fails if owner cannot receive ETH', async () => {
        ...
        // Alice sends LAI to NonPayable so its LAI balance covers its debt
        await laiToken.transfer(nonPayable.address, dec(10000, 18), {from: alice})
...
/* TODO:
 ...
 changes with addColl, withdrawColl, withdrawLAI, repayLAI, etc. Can split them up and put them with
