...
contract('CollSurplusPool', async accounts => {
  ...
  it("CollSurplusPool::getETH(): Returns the ETH balance of the CollSurplusPool after redemption", async () => {
    ...
    await openTrove({ extraLAIAmount: B_netDebt, extraParams: { from: A, value: dec(3000, 'ether') } })
  ...
  it("CollSurplusPool: claimColl(): Reverts if owner cannot receive ETH surplus", async () => {
    ...
    const B_laiAmount = toBN(dec(3000, 18))
    const B_netDebt = await th.getAmountWithBorrowingFee(contracts, B_laiAmount)
    const openTroveData = th.getTransactionData('openTrove(uint256,uint256,address,address)', ['0xde0b6b3a7640000', web3.utils.toHex(B_laiAmount), B, B])
