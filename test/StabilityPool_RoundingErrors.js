...
contract('Pool Manager: Sum-Product rounding errors', async accounts => {
  ...
  it.skip("Rounding errors: 100 deposits of 100LAI into SP, then 200 liquidations of 49LAI", async () => {
    ...
    const SP_TotalDeposits = await stabilityPool.getTotalLAIDeposits()
    const SP_ETH = await stabilityPool.getETH()
    const compoundedDeposit = await stabilityPool.getCompoundedLAIDeposit(depositors[0])
