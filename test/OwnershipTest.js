...
contract('All Liquity functions with onlyOwner modifier', async accounts => {
  ...
  describe('CommunityIssuance', async accounts => {
    it("setAddresses(): reverts when called by non-owner, with wrong addresses, or twice", async () => {
      const params = [laoToken.address, stabilityPool.address]
  ...
  describe('LAOStaking', async accounts => {
    it("setAddresses(): reverts when called by non-owner, with wrong addresses, or twice", async () => {
      await testSetAddresses(laoStaking, 5)
    })
  })

  describe('LockupContractFactory', async accounts => {
    it("setLAOAddress(): reverts when called by non-owner, with wrong address, or twice", async () => {
      await th.assertRevert(lockupContractFactory.setLAOTokenAddress(laoToken.address, { from: alice }))

      const params = [laoToken.address]

      // Attempt to use zero address
      await testZeroAddress(lockupContractFactory, params, 'setLAOTokenAddress')
      // Attempt to use non contract
      await testNonContractAddress(lockupContractFactory, params, 'setLAOTokenAddress')

      // Owner can successfully set any address
      const txOwner = await lockupContractFactory.setLAOTokenAddress(laoToken.address, { from: owner })

      assert.isTrue(txOwner.receipt.status)
      // fails if called twice
      await th.assertRevert(lockupContractFactory.setLAOTokenAddress(laoToken.address, { from: owner }))
    })
  })
})
