...
contract('TroveManager', async accounts => {
  ...
  const getOpenTroveTotalDebt = async (laiAmount) => th.getOpenTroveTotalDebt(contracts, laiAmount)
  ...
  const withdrawLAI = async (params) => th.withdrawLAI(contracts, params)
  ...
  it('liquidate(): closes a Trove that has ICR < MCR', async () => {
    ...
    // Alice increases debt to 180 LAI, lowering her ICR to 1.11
    const A_LAIWithdrawal = await getNetBorrowingAmount(dec(130, 18))

    const targetICR = toBN('1111111111111111111')
    await withdrawLAI({ ICR: targetICR, extraParams: { from: alice } })
    ...
    // price drops to 1ETH:100LAI, reducing Alice's ICR below MCR
  ...
  it("liquidate(): decreases ActivePool ETH and LAIDebt by correct amounts", async () => {
    // --- SETUP ---
    ...
    // check ActivePool ETH and LAI debt before
    ...
    const activePool_LAIDebt_Before = (await activePool.getLAIDebt()).toString()
    ...
    th.assertIsApproximatelyEqual(activePool_LAIDebt_Before, A_totalDebt.add(B_totalDebt))

    // price drops to 1ETH:100LAI, reducing Bob's ICR below MCR
    await priceFeed.setPrice('100000000000000000000');

    // Confirm system is not in Recovery Mode
    assert.isFalse(await th.checkRecoveryMode(contracts));

    /* close Bob's Trove. Should liquidate his ether and LAI, 
    leaving Alice’s ether and LAI debt in the ActivePool. */
    await troveManager.liquidate(bob, { from: owner });

    // check ActivePool ETH and LAI debt 
    ...
    const activePool_LAIDebt_After = (await activePool.getLAIDebt()).toString()

    assert.equal(activePool_ETH_After, A_collateral)
    assert.equal(activePool_RawEther_After, A_collateral)
    th.assertIsApproximatelyEqual(activePool_LAIDebt_After, A_totalDebt)
  })

  it("liquidate(): increases DefaultPool ETH and LAI debt by correct amounts", async () => {
    // --- SETUP ---
    const { collateral: A_collateral, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(4, 18)), extraParams: { from: alice } })
    const { collateral: B_collateral, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(21, 17)), extraParams: { from: bob } })
    ...
    // check DefaultPool ETH and LAI debt before
    ...
    const defaultPool_LAIDebt_Before = (await defaultPool.getLAIDebt()).toString()

    assert.equal(defaultPool_ETH_Before, '0')
    assert.equal(defaultPool_RawEther_Before, '0')
    assert.equal(defaultPool_LAIDebt_Before, '0')
    ...
    const defaultPool_LAIDebt_After = (await defaultPool.getLAIDebt()).toString()
    ...
    th.assertIsApproximatelyEqual(defaultPool_LAIDebt_After, B_totalDebt)
  })
  ...
  it("liquidate(): updates the snapshots of total stakes and total collateral", async () => {
    ...
    // close Bob's Trove.  His ether*0.995 and LAI should be added to the DefaultPool.
    await troveManager.liquidate(bob, { from: owner });
  ...
  it("liquidate(): updates the L_ETH and L_LAIDebt reward-per-unit-staked totals", async () => {
    // --- SETUP ---
    ...
    // price drops to 1ETH:100LAI, reducing Carols's ICR below MCR
    await priceFeed.setPrice('100000000000000000000');
    ...
    // Carol's ether*0.995 and LAI should be added to the DefaultPool.
    const L_ETH_AfterCarolLiquidated = await troveManager.L_ETH()
    const L_LAIDebt_AfterCarolLiquidated = await troveManager.L_LAIDebt()

    const L_ETH_expected_1 = th.applyLiquidationFee(C_collateral).mul(mv._1e18BN).div(A_collateral.add(B_collateral))
    const L_LAIDebt_expected_1 = C_totalDebt.mul(mv._1e18BN).div(A_collateral.add(B_collateral))
    assert.isAtMost(th.getDifference(L_ETH_AfterCarolLiquidated, L_ETH_expected_1), 100)
    assert.isAtMost(th.getDifference(L_LAIDebt_AfterCarolLiquidated, L_LAIDebt_expected_1), 100)

    // Bob now withdraws LAI, bringing his ICR to 1.11
    const { increasedTotalDebt: B_increasedTotalDebt } = await withdrawLAI({ ICR: toBN(dec(111, 16)), extraParams: { from: bob } })

    // Confirm system is not in Recovery Mode
    assert.isFalse(await th.checkRecoveryMode(contracts));

    // price drops to 1ETH:50LAI, reducing Bob's ICR below MCR
   ...
   L_LAIDebt = (180 / 20) + (890 / 10) = 98 LAI */
    const L_ETH_AfterBobLiquidated = await troveManager.L_ETH()
    const L_LAIDebt_AfterBobLiquidated = await troveManager.L_LAIDebt()
    ...
    const L_LAIDebt_expected_2 = L_LAIDebt_expected_1.add(B_totalDebt.add(B_increasedTotalDebt).add(B_collateral.mul(L_LAIDebt_expected_1).div(mv._1e18BN)).mul(mv._1e18BN).div(A_collateral))
    assert.isAtMost(th.getDifference(L_ETH_AfterBobLiquidated, L_ETH_expected_2), 100)
    assert.isAtMost(th.getDifference(L_LAIDebt_AfterBobLiquidated, L_LAIDebt_expected_2), 100)
  })

  it("liquidate(): Liquidates undercollateralized trove if there are two troves in the system", async () => {
    await openTrove({ ICR: toBN(dec(200, 18)), extraParams: { from: bob, value: dec(100, 'ether') } })

    // Alice creates a single trove with 0.7 ETH and a debt of 70 LAI, and provides 10 LAI to SP
    const { collateral: A_collateral, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })

    // Alice proves 10 LAI to SP
  ...
  it("liquidate(): Given the same price and no other trove changes, complete Pool offsets restore the TCR to its value prior to the defaulters opening troves", async () => {
    // Whale provides LAI to SP
    const spDeposit = toBN(dec(100, 24))
    await openTrove({ ICR: toBN(dec(4, 18)), extraLAIAmount: spDeposit, extraParams: { from: whale } })
  ...
  it("liquidate(): does not affect the SP deposit or ETH gain when called on an SP depositor's address that has no trove", async () => {
    ...
    await openTrove({ ICR: toBN(dec(3, 18)), extraLAIAmount: spDeposit, extraParams: { from: bob } })
    const { C_totalDebt, C_collateral } = await openTrove({ ICR: toBN(dec(218, 16)), extraLAIAmount: toBN(dec(100, 18)), extraParams: { from: carol } })

    // Bob sends tokens to Dennis, who has no trove
    await laiToken.transfer(dennis, spDeposit, { from: bob })

    //Dennis provides LAI to SP
    ...
    const dennis_Deposit_Before = (await stabilityPool.getCompoundedLAIDeposit(dennis)).toString()
    ...
    const dennis_Deposit_After = (await stabilityPool.getCompoundedLAIDeposit(dennis)).toString()
  ...
  it("liquidate(): does not liquidate a SP depositor's trove with ICR > 110%, and does not affect their SP deposit or ETH gain", async () => {
    ...
    const spDeposit = toBN(dec(1, 24))
    await openTrove({ ICR: toBN(dec(3, 18)), extraLAIAmount: spDeposit, extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(218, 16)), extraLAIAmount: toBN(dec(100, 18)), extraParams: { from: carol } })

    //Bob provides LAI to SP
    ...
    const bob_Deposit_Before = (await stabilityPool.getCompoundedLAIDeposit(bob)).toString()
    ...
    const bob_Deposit_After = (await stabilityPool.getCompoundedLAIDeposit(bob)).toString()
  ...
  it("liquidate(): liquidates a SP depositor's trove with ICR < 110%, and the liquidation correctly impacts their SP deposit and ETH gain", async () => {
    ...
    await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: whale } })
    await openTrove({ ICR: toBN(dec(8, 18)), extraLAIAmount: A_spDeposit, extraParams: { from: alice } })
    const { collateral: B_collateral, totalDebt: B_debt } = await openTrove({ ICR: toBN(dec(218, 16)), extraLAIAmount: B_spDeposit, extraParams: { from: bob } })
    const { collateral: C_collateral, totalDebt: C_debt } = await openTrove({ ICR: toBN(dec(210, 16)), extraLAIAmount: toBN(dec(100, 18)), extraParams: { from: carol } })
    ...
    // Check Bob' SP deposit has absorbed Carol's debt, and he has received her liquidated ETH
    const bob_Deposit_Before = await stabilityPool.getCompoundedLAIDeposit(bob)
    ...
    /* Alice's LAI Loss = (300 / 400) * 200 = 150 LAI
       Alice's ETH gain = (300 / 400) * 2*0.995 = 1.4925 ETH

       Bob's LAILoss = (100 / 400) * 200 = 50 LAI
       Bob's ETH gain = (100 / 400) * 2*0.995 = 0.4975 ETH

     Check Bob' SP deposit has been reduced to 50 LAI, and his ETH gain has increased to 1.5 ETH. */
    const alice_Deposit_After = (await stabilityPool.getCompoundedLAIDeposit(alice)).toString()
    ...
    const bob_Deposit_After = await stabilityPool.getCompoundedLAIDeposit(bob)
  ...
  it("liquidate(): does not alter the liquidated user's token balance", async () => {
    await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
    const { laiAmount: A_laiAmount } = await openTrove({ ICR: toBN(dec(2, 18)), extraLAIAmount: toBN(dec(300, 18)), extraParams: { from: alice } })
    const { laiAmount: B_laiAmount } = await openTrove({ ICR: toBN(dec(2, 18)), extraLAIAmount: toBN(dec(200, 18)), extraParams: { from: bob } })
    const { laiAmount: C_laiAmount } = await openTrove({ ICR: toBN(dec(2, 18)), extraLAIAmount: toBN(dec(100, 18)), extraParams: { from: carol } })
    ...
    // Liquidate A, B and C
    const activeLAIDebt_0 = await activePool.getLAIDebt()
    const defaultLAIDebt_0 = await defaultPool.getLAIDebt()

    await troveManager.liquidate(alice)
    const activeLAIDebt_A = await activePool.getLAIDebt()
    const defaultLAIDebt_A = await defaultPool.getLAIDebt()

    await troveManager.liquidate(bob)
    const activeLAIDebt_B = await activePool.getLAIDebt()
    const defaultLAIDebt_B = await defaultPool.getLAIDebt()
    ...
    assert.equal((await laiToken.balanceOf(alice)).toString(), A_laiAmount)
    assert.equal((await laiToken.balanceOf(bob)).toString(), B_laiAmount)
    assert.equal((await laiToken.balanceOf(carol)).toString(), C_laiAmount)
  })

  it("liquidate(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    await openTrove({ ICR: toBN(dec(8, 18)), extraLAIAmount: toBN(dec(100, 18)), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(221, 16)), extraLAIAmount: toBN(dec(100, 18)), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(2, 18)), extraLAIAmount: toBN(dec(100, 18)), extraParams: { from: carol } })

    // Defaulter opens with 60 LAI, 0.6 ETH
    ...
    /* Liquidate defaulter. 30 LAI and 0.3 ETH is distributed between A, B and C.

    A receives (30 * 2/4) = 15 LAI, and (0.3*2/4) = 0.15 ETH
    B receives (30 * 1/4) = 7.5 LAI, and (0.3*1/4) = 0.075 ETH
    C receives (30 * 1/4) = 7.5 LAI, and (0.3*1/4) = 0.075 ETH
    */
    await troveManager.liquidate(defaulter_1)
  ...
  it("liquidate(): when SP > 0, triggers LAO reward event - increases the sum G", async () => {
    ...
    assert.equal(await stabilityPool.getTotalLAIDeposits(), dec(100, 18))

    const G_Before = await stabilityPool.epochToScaleToG(0, 0)

    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_HOUR, web3.currentProvider)

    // Price drops to 1ETH:100LAI, reducing defaulters to below MCR
    await priceFeed.setPrice(dec(100, 18));
    const price = await priceFeed.getPrice()
    assert.isFalse(await th.checkRecoveryMode(contracts))
    ...
    // Expect G has increased from the LAO reward event triggered
  ...
  it("liquidate(): when SP is empty, doesn't update G", async () => {
    ...
    // Check SP is empty
    assert.equal((await stabilityPool.getTotalLAIDeposits()), '0')
  ...
  it('liquidateTroves(): liquidates a Trove that a) was skipped in a previous liquidation and b) has pending rewards', async () => {
    ...
    // A adds 10 LAI to the SP, but less than C's debt
    ...
    await borrowerOperations.repayLAI(dec(1, 18), D, D, {from: D})
    await borrowerOperations.repayLAI(dec(1, 18), E, E, {from: E})
    ...
    const pendingLAIDebt_C = await troveManager.getPendingLAIDebtReward(C)
    const defaultPoolETH = await defaultPool.getETH()
    const defaultPoolLAIDebt = await defaultPool.getLAIDebt()
    assert.isTrue(pendingETH_C.lte(defaultPoolETH))
    assert.isTrue(pendingLAIDebt_C.lte(defaultPoolLAIDebt))
    //Check only difference is dust
    assert.isAtMost(th.getDifference(pendingETH_C, defaultPoolETH), 1000)
    assert.isAtMost(th.getDifference(pendingLAIDebt_C, defaultPoolLAIDebt), 1000)
  ...
  it('liquidateTroves(): closes every Trove with ICR < MCR, when n > number of undercollateralized troves', async () => {
    ...
    // Price drops to 1ETH:100LAI, reducing Bob and Carol's ICR below MCR
    await priceFeed.setPrice(dec(100, 18));
    const price = await priceFeed.getPrice()
  ...
  it("liquidateTroves(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    ...
    // Liquidate defaulter. 30 LAI and 0.3 ETH is distributed uniformly between A, B and C. Each receive 10 LAI, 0.1 ETH
    await troveManager.liquidate(defaulter_1)
    ...
    // Whale enters system, pulling it into Normal Mode
    await openTrove({ ICR: toBN(dec(10, 18)), extraLAIAmount: dec(1, 24), extraParams: { from: whale } })
  ...
  it("liquidateTroves(): does not affect the liquidated user's token balances", async () => {
    ...
    const D_balanceBefore = await laiToken.balanceOf(dennis)
    const E_balanceBefore = await laiToken.balanceOf(erin)
    const F_balanceBefore = await laiToken.balanceOf(flyn)
    ...
    assert.equal((await laiToken.balanceOf(dennis)).toString(), D_balanceBefore)
    assert.equal((await laiToken.balanceOf(erin)).toString(), E_balanceBefore)
    assert.equal((await laiToken.balanceOf(flyn)).toString(), F_balanceBefore)
  })

  it("liquidateTroves(): A liquidation sequence containing Pool offsets increases the TCR", async () => {
    // Whale provides 500 LAI to SP
    await openTrove({ ICR: toBN(dec(100, 18)), extraLAIAmount: toBN(dec(500, 18)), extraParams: { from: whale } })
    ...
    // Check pool has 500 LAI
    assert.equal((await stabilityPool.getTotalLAIDeposits()).toString(), dec(500, 18))
    ...
    assert.equal((await stabilityPool.getTotalLAIDeposits()).toString(), '0')
  ...
  it("liquidateTroves(): Liquidating troves with SP deposits correctly impacts their SP deposit and ETH gain", async () => {
    // Whale provides 400 LAI to the SP
    const whaleDeposit = toBN(dec(40000, 18))
    await openTrove({ ICR: toBN(dec(100, 18)), extraLAIAmount: whaleDeposit, extraParams: { from: whale } })
    ...
    const { collateral: A_coll, totalDebt: A_debt } = await openTrove({ ICR: toBN(dec(2, 18)), extraLAIAmount: A_deposit, extraParams: { from: alice } })
    const { collateral: B_coll, totalDebt: B_debt } = await openTrove({ ICR: toBN(dec(2, 18)), extraLAIAmount: B_deposit, extraParams: { from: bob } })
    ...
    // Check 800 LAI in Pool
    const totalDeposits = whaleDeposit.add(A_deposit).add(B_deposit)
    assert.equal((await stabilityPool.getTotalLAIDeposits()).toString(), totalDeposits)
    ...
    Total LAI in Pool: 800 LAI

    Then, liquidation hits A,B,C: 

    Total liquidated debt = 150 + 350 + 150 = 650 LAI
    Total liquidated ETH = 1.1 + 3.1 + 1.1 = 5.3 ETH

    whale lai loss: 650 * (400/800) = 325 lai
    alice lai loss:  650 *(100/800) = 81.25 lai
    bob lai loss: 650 * (300/800) = 243.75 lai

    whale remaining deposit: (400 - 325) = 75 lai
    alice remaining deposit: (100 - 81.25) = 18.75 lai
    bob remaining deposit: (300 - 243.75) = 56.25 lai
    ...
    Total remaining deposits: 150 LAI
    Total ETH gain: 4.975 ETH */

    // Check remaining LAI Deposits and ETH gain, for whale and depositors whose troves were liquidated
    const whale_Deposit_After = await stabilityPool.getCompoundedLAIDeposit(whale)
    const alice_Deposit_After = await stabilityPool.getCompoundedLAIDeposit(alice)
    const bob_Deposit_After = await stabilityPool.getCompoundedLAIDeposit(bob)
    ...
    const total_LAIinSP = (await stabilityPool.getTotalLAIDeposits()).toString()
    const total_ETHinSP = (await stabilityPool.getETH()).toString()

    assert.isAtMost(th.getDifference(total_LAIinSP, totalDeposits.sub(liquidatedDebt)), 1000)
    assert.isAtMost(th.getDifference(total_ETHinSP, th.applyLiquidationFee(liquidatedColl)), 1000)
  })

  it("liquidateTroves(): when SP > 0, triggers LAO reward event - increases the sum G", async () => {
    await openTrove({ ICR: toBN(dec(100, 18)), extraParams: { from: whale } })

    // A, B, C open troves
    await openTrove({ ICR: toBN(dec(4, 18)), extraParams: { from: A } })
    await openTrove({ ICR: toBN(dec(3, 18)), extraLAIAmount: toBN(dec(100, 18)), extraParams: { from: B } })
    await openTrove({ ICR: toBN(dec(3, 18)), extraParams: { from: C } })
  ...
  it('batchLiquidateTroves(): closes every trove with ICR < MCR in the given array', async () => {
    ...
    // Price drops to 1ETH:100LAI, reducing A, B, C ICR below MCR
    await priceFeed.setPrice(dec(100, 18));
    const price = await priceFeed.getPrice()
  ...
  it('batchLiquidateTroves(): does not liquidate troves that are not in the given array', async () => {
    ...
    await openTrove({ ICR: toBN(dec(200, 16)), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(200, 16)), extraLAIAmount: toBN(dec(500, 18)), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(200, 16)), extraLAIAmount: toBN(dec(500, 18)), extraParams: { from: erin } })
  ...
  it("batchLiquidateTroves(): skips if trove is non-existent", async () => {
    ...
    await openTrove({ ICR: toBN(dec(100, 18)), extraLAIAmount: spDeposit, extraParams: { from: whale } })
    ...
    th.assertIsApproximatelyEqual((await stabilityPool.getTotalLAIDeposits()).toString(), spDeposit.sub(A_debt).sub(B_debt))
  ...
  it("batchLiquidateTroves(): skips if a trove has been closed", async () => {
    ...
    await laiToken.transfer(carol, dec(100, 18), { from: whale })
  ...
  it("batchLiquidateTroves: when SP > 0, triggers LAO reward event - increases the sum G", async () => {
    await openTrove({ ICR: toBN(dec(100, 18)), extraParams: { from: whale } })
  ...
  it('getRedemptionHints(): gets the address of the first Trove and the final ICR of the last Trove involved in a redemption', async () => {
    ...
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(310, 16)), extraLAIAmount: partialRedemptionAmount, extraParams: { from: alice } })
  ...
  it('getRedemptionHints(): returns 0 as partialRedemptionHintNICR when reaching _maxIterations', async () => {
    ...
    // Get hints for a redemption of 170 + 30 + some extra LAI. At least 3 iterations are needed
  ...
  it('redeemCollateral(): cancels the provided LAI with debt from Troves with the lowest ICRs and sends an equivalent amount of Ether', async () => {
    // --- SETUP ---
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(310, 16)), extraLAIAmount: dec(10, 18), extraParams: { from: alice } })
    const { netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(290, 16)), extraLAIAmount: dec(8, 18), extraParams: { from: bob } })
    const { netDebt: C_netDebt } = await openTrove({ ICR: toBN(dec(250, 16)), extraLAIAmount: dec(10, 18), extraParams: { from: carol } })
    const partialRedemptionAmount = toBN(2)
    const redemptionAmount = C_netDebt.add(B_netDebt).add(partialRedemptionAmount)
    // start Dennis with a high ICR
    await openTrove({ ICR: toBN(dec(100, 18)), extraLAIAmount: redemptionAmount, extraParams: { from: dennis } })

    const dennis_ETHBalance_Before = toBN(await web3.eth.getBalance(dennis))

    const dennis_LAIBalance_Before = await laiToken.balanceOf(dennis)
    ...
    // Find hints for redeeming 20 LAI
    const {
      firstRedemptionHint,
      partialRedemptionHintNICR
    } = await hintHelpers.getRedemptionHints(redemptionAmount, price, 0)
    ...
    /* check that Dennis' redeemed 20 LAI has been cancelled with debt from Bobs's Trove (8) and Carol's Trove (10).
    The remaining lot (2) is sent to Alice's Trove, who had the best ICR.
    It leaves her with (3) LAI debt + 50 for gas compensation. */
    ...
    const expectedTotalETHDrawn = redemptionAmount.div(toBN(200)) // convert redemptionAmount LAI to ETH, at ETH:USD price 200
    ...
    const dennis_LAIBalance_After = (await laiToken.balanceOf(dennis)).toString()
    assert.equal(dennis_LAIBalance_After, dennis_LAIBalance_Before.sub(redemptionAmount))
  })
  ...
  it('redeemCollateral(): ends the redemption sequence when the token redemption request has been filled', async () => {
    ...
    const { netDebt: A_debt } = await openTrove({ ICR: toBN(dec(290, 16)), extraLAIAmount: dec(20, 18), extraParams: { from: alice } })
    const { netDebt: B_debt } = await openTrove({ ICR: toBN(dec(290, 16)), extraLAIAmount: dec(20, 18), extraParams: { from: bob } })
    const { netDebt: C_debt } = await openTrove({ ICR: toBN(dec(290, 16)), extraLAIAmount: dec(20, 18), extraParams: { from: carol } })
    const redemptionAmount = A_debt.add(B_debt).add(C_debt)
    const { totalDebt: D_totalDebt, collateral: D_coll } = await openTrove({ ICR: toBN(dec(300, 16)), extraLAIAmount: dec(10, 18), extraParams: { from: dennis } })
    const { totalDebt: E_totalDebt, collateral: E_coll } = await openTrove({ ICR: toBN(dec(300, 16)), extraLAIAmount: dec(10, 18), extraParams: { from: erin } })

    // --- TEST --- 

    // open trove from redeemer.  Redeemer has highest ICR (100ETH, 100 LAI), 20000%
    const { laiAmount: F_laiAmount } = await openTrove({ ICR: toBN(dec(200, 18)), extraLAIAmount: redemptionAmount.mul(toBN(2)), extraParams: { from: flyn } })
    ...
    // Check Flyn's redemption has reduced his balance from 100 to (100-60) = 40 LAI
    const flynBalance = await laiToken.balanceOf(flyn)
    th.assertIsApproximatelyEqual(flynBalance, F_laiAmount.sub(redemptionAmount))
  ...
  it('redeemCollateral(): ends the redemption sequence when max iterations have been reached', async () => {
    ...
    const { netDebt: A_debt } = await openTrove({ ICR: toBN(dec(286, 16)), extraLAIAmount: dec(20, 18), extraParams: { from: alice } })
    const { netDebt: B_debt } = await openTrove({ ICR: toBN(dec(286, 16)), extraLAIAmount: dec(20, 18), extraParams: { from: bob } })
    const { netDebt: C_debt, totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(286, 16)), extraLAIAmount: dec(20, 18), extraParams: { from: carol } })
    ...
    // Check Flyn's redemption has reduced his balance from 100 to (100-40) = 60 LAI
    const flynBalance = (await laiToken.balanceOf(flyn)).toString()
    th.assertIsApproximatelyEqual(flynBalance, F_laiAmount.sub(redemptionAmount))
  ...
  it("redeemCollateral(): performs partial redemption if resultant debt is > minimum net debt", async () => {
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveLAIAmount(dec(10000, 18)), A, A, { from: A, value: dec(1000, 'ether') })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveLAIAmount(dec(20000, 18)), B, B, { from: B, value: dec(1000, 'ether') })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveLAIAmount(dec(30000, 18)), C, C, { from: C, value: dec(1000, 'ether') })

    // A and C send all their tokens to B
    await laiToken.transfer(B, await laiToken.balanceOf(A), {from: A})
    await laiToken.transfer(B, await laiToken.balanceOf(C), {from: C})
    ...
    // LAI redemption is 55000 US
    const LAIRedemption = dec(55000, 18)
    const tx1 = await th.redeemCollateralAndGetTxObject(B, contracts, LAIRedemption, th._100pct)
  ...
  it("redeemCollateral(): doesn't perform partial redemption if resultant debt would be < minimum net debt", async () => {
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveLAIAmount(dec(6000, 18)), A, A, { from: A, value: dec(1000, 'ether') })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveLAIAmount(dec(20000, 18)), B, B, { from: B, value: dec(1000, 'ether') })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveLAIAmount(dec(30000, 18)), C, C, { from: C, value: dec(1000, 'ether') })
  ...
  it('redeemCollateral(): doesnt perform the final partial redemption in the sequence if the hint is out-of-date', async () => {
    // --- SETUP ---
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(363, 16)), extraLAIAmount: dec(5, 18), extraParams: { from: alice } })
    const { netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(344, 16)), extraLAIAmount: dec(8, 18), extraParams: { from: bob } })
    const { netDebt: C_netDebt } = await openTrove({ ICR: toBN(dec(333, 16)), extraLAIAmount: dec(10, 18), extraParams: { from: carol } })
    ...
    // Oops, another transaction gets in the way
    {
      ...
      // Alice redeems 1 LAI from Carol's Trove
    ...
    // Since Alice already redeemed 1 LAI from Carol's Trove, Dennis was  able to redeem:
    //  - 9 LAI from Carol's
    //  - 8 LAI from Bob's
    // for a total of 17 LAI.

    // Dennis calculated his hint for redeeming 2 LAI from Alice's Trove, but after Alice's transaction
    // got in the way, he would have needed to redeem 3 LAI to fully complete his redemption of 20 LAI.
    ...
    const expectedTotalETHDrawn = fullfilledRedemptionAmount.sub(frontRunRedepmtion).div(toBN(200)) // redempted LAI converted to ETH, at ETH:USD price 200
    const expectedReceivedETH = expectedTotalETHDrawn.sub(ETHFee)

    th.assertIsApproximatelyEqual(expectedReceivedETH, receivedETH)

    const dennis_LAIBalance_After = (await laiToken.balanceOf(dennis)).toString()
    th.assertIsApproximatelyEqual(dennis_LAIBalance_After, dennis_LAIBalance_Before.sub(fullfilledRedemptionAmount.sub(frontRunRedepmtion)))
  })
  ...
  it.skip("redeemCollateral(): can redeem if there is zero active debt but non-zero debt in DefaultPool", async () => {
    ...
    const amount = await getOpenTroveLAIAmount(dec(110, 18))
    await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(133, 16)), extraLAIAmount: amount, extraParams: { from: bob } })

    await laiToken.transfer(carol, amount, { from: bob })
    ...
    const expectedTotalETHDrawn = toBN(amount).div(toBN(100)) // convert 100 LAI to ETH at ETH:USD price of 100
    const expectedReceivedETH = expectedTotalETHDrawn.sub(ETHFee)

    const receivedETH = carol_ETHBalance_After.sub(carol_ETHBalance_Before)
    assert.isTrue(expectedReceivedETH.eq(receivedETH))

    const carol_LAIBalance_After = (await laiToken.balanceOf(carol)).toString()
    assert.equal(carol_LAIBalance_After, '0')
  })
  ...
  it("redeemCollateral(): doesn't touch Troves with ICR < 110%", async () => {
    ...
    const { laiAmount: B_laiAmount, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(133, 16)), extraLAIAmount: A_debt, extraParams: { from: bob } })

    await laiToken.transfer(carol, B_laiAmount, { from: bob })
  ...
  it("redeemCollateral(): finds the last Trove with ICR == 110% even if there is more than one", async () => {
    ...
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraLAIAmount: amount1, extraParams: { from: alice } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraLAIAmount: amount1, extraParams: { from: bob } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraLAIAmount: amount1, extraParams: { from: carol } })
    const redemptionAmount = C_totalDebt.add(B_totalDebt).add(A_totalDebt)
    const { totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(195, 16)), extraLAIAmount: redemptionAmount, extraParams: { from: dennis } })
    ...
    await openTrove({ ICR: toBN(dec(100, 18)), extraLAIAmount: dec(10, 18), extraParams: { from: whale } })
  ...
  it("redeemCollateral(): reverts when argument _amount is 0", async () => {
    ...
    // Alice opens trove and transfers 500LAI to Erin, the would-be redeemer
    await openTrove({ ICR: toBN(dec(200, 16)), extraLAIAmount: dec(500, 18), extraParams: { from: alice } })
    await laiToken.transfer(erin, dec(500, 18), { from: alice })
  ...
  it("redeemCollateral(): reverts if max fee > 100%", async () => {
    await openTrove({ ICR: toBN(dec(400, 16)), extraLAIAmount: dec(10, 18), extraParams: { from: A } })
    await openTrove({ ICR: toBN(dec(400, 16)), extraLAIAmount: dec(20, 18), extraParams: { from: B } })
    await openTrove({ ICR: toBN(dec(400, 16)), extraLAIAmount: dec(30, 18), extraParams: { from: C } })
    await openTrove({ ICR: toBN(dec(400, 16)), extraLAIAmount: dec(40, 18), extraParams: { from: D } })
  ...
  it("redeemCollateral(): reverts if fee exceeds max fee percentage", async () => {
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraLAIAmount: dec(80, 18), extraParams: { from: A } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraLAIAmount: dec(90, 18), extraParams: { from: B } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraLAIAmount: dec(100, 18), extraParams: { from: C } })
    const expectedTotalSupply = A_totalDebt.add(B_totalDebt).add(C_totalDebt)

    // Check total LAI supply
    ...
    // LAI redemption is 27 USD: a redemption that incurs a fee of 27/(270 * 2) = 5%
    const attemptedLAIRedemption = expectedTotalSupply.div(toBN(10))

    // Max fee is <5%
    const lessThan5pct = '49999999999999999'
    await assertRevert(th.redeemCollateralAndGetTxObject(A, contracts, attemptedLAIRedemption, lessThan5pct), "Fee exceeded provided maximum")
  
    await troveManager.setBaseRate(0)  // artificially zero the baseRate
    
    // Max fee is 1%
    await assertRevert(th.redeemCollateralAndGetTxObject(A, contracts, attemptedLAIRedemption, dec(1, 16)), "Fee exceeded provided maximum")
  
    await troveManager.setBaseRate(0)

     // Max fee is 3.754%
    await assertRevert(th.redeemCollateralAndGetTxObject(A, contracts, attemptedLAIRedemption, dec(3754, 13)), "Fee exceeded provided maximum")
  
    await troveManager.setBaseRate(0)

    // Max fee is 0.5%
    await assertRevert(th.redeemCollateralAndGetTxObject(A, contracts, attemptedLAIRedemption, dec(5, 15)), "Fee exceeded provided maximum")
  })

  it("redeemCollateral(): succeeds if fee is less than max fee percentage", async () => {
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraLAIAmount: dec(9500, 18), extraParams: { from: A } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(395, 16)), extraLAIAmount: dec(9000, 18), extraParams: { from: B } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(390, 16)), extraLAIAmount: dec(10000, 18), extraParams: { from: C } })
    const expectedTotalSupply = A_totalDebt.add(B_totalDebt).add(C_totalDebt)
    ...
    // LAI redemption fee with 10% of the supply will be 0.5% + 1/(10*2)
    const attemptedLAIRedemption = expectedTotalSupply.div(toBN(10))

    // Attempt with maxFee > 5.5%
    const price = await priceFeed.getPrice()
    const ETHDrawn = attemptedLAIRedemption.mul(mv._1e18BN).div(price)
    const slightlyMoreThanFee = (await troveManager.getRedemptionFeeWithDecay(ETHDrawn))
    const tx1 = await th.redeemCollateralAndGetTxObject(A, contracts, attemptedLAIRedemption, slightlyMoreThanFee)
    ...
    const tx2 = await th.redeemCollateralAndGetTxObject(C, contracts, attemptedLAIRedemption, exactSameFee)
    assert.isTrue(tx2.receipt.status)

    await troveManager.setBaseRate(0)

     // Max fee is 10%
    const tx3 = await th.redeemCollateralAndGetTxObject(B, contracts, attemptedLAIRedemption, dec(1, 17))
    assert.isTrue(tx3.receipt.status)

    await troveManager.setBaseRate(0)

    // Max fee is 37.659%
    const tx4 = await th.redeemCollateralAndGetTxObject(A, contracts, attemptedLAIRedemption, dec(37659, 13))
    assert.isTrue(tx4.receipt.status)

    await troveManager.setBaseRate(0)

    // Max fee is 100%
    const tx5 = await th.redeemCollateralAndGetTxObject(C, contracts, attemptedLAIRedemption, dec(1, 18))
  ...
  it("redeemCollateral(): doesn't affect the Stability Pool deposits or ETH gain of redeemed-from troves", async () => {
    ...
    // B, C, D, F open trove
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraLAIAmount: dec(100, 18), extraParams: { from: bob } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(195, 16)), extraLAIAmount: dec(200, 18), extraParams: { from: carol } })
    const { totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(190, 16)), extraLAIAmount: dec(400, 18), extraParams: { from: dennis } })
    const { totalDebt: F_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraLAIAmount: dec(100, 18), extraParams: { from: flyn } })

    const redemptionAmount = B_totalDebt.add(C_totalDebt).add(D_totalDebt).add(F_totalDebt)
    // Alice opens trove and transfers LAI to Erin, the would-be redeemer
    await openTrove({ ICR: toBN(dec(300, 16)), extraLAIAmount: redemptionAmount, extraParams: { from: alice } })
    await laiToken.transfer(erin, redemptionAmount, { from: alice })
    ...
    const bob_SPDeposit_before = (await stabilityPool.getCompoundedLAIDeposit(bob)).toString()
    const carol_SPDeposit_before = (await stabilityPool.getCompoundedLAIDeposit(carol)).toString()
    const dennis_SPDeposit_before = (await stabilityPool.getCompoundedLAIDeposit(dennis)).toString()

    const bob_ETHGain_before = (await stabilityPool.getDepositorETHGain(bob)).toString()
    const carol_ETHGain_before = (await stabilityPool.getDepositorETHGain(carol)).toString()
    const dennis_ETHGain_before = (await stabilityPool.getDepositorETHGain(dennis)).toString()

    // Check the remaining LAI and ETH in Stability Pool after liquidation is non-zero
    const LAIinSP = await stabilityPool.getTotalLAIDeposits()
    const ETHinSP = await stabilityPool.getETH()
    assert.isTrue(LAIinSP.gte(mv._zeroBN))
    ...
    const bob_SPDeposit_after = (await stabilityPool.getCompoundedLAIDeposit(bob)).toString()
    const carol_SPDeposit_after = (await stabilityPool.getCompoundedLAIDeposit(carol)).toString()
    const dennis_SPDeposit_after = (await stabilityPool.getCompoundedLAIDeposit(dennis)).toString()
  ...
  it("redeemCollateral(): caller can redeem their entire LAIToken balance", async () => {
    const { collateral: W_coll, totalDebt: W_totalDebt } = await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: whale } })

    // Alice opens trove and transfers 400 LAI to Erin, the would-be redeemer
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(300, 16)), extraLAIAmount: dec(400, 18), extraParams: { from: alice } })
    await laiToken.transfer(erin, dec(400, 18), { from: alice })

    // Check Erin's balance before
    const erin_balance_before = await laiToken.balanceOf(erin)
    assert.equal(erin_balance_before, dec(400, 18))

    // B, C, D open trove
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(300, 16)), extraLAIAmount: dec(590, 18), extraParams: { from: bob } })
    const { collateral: C_coll, totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(300, 16)), extraLAIAmount: dec(1990, 18), extraParams: { from: carol } })
    const { collateral: D_coll, totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(500, 16)), extraLAIAmount: dec(1990, 18), extraParams: { from: dennis } })
    ...
    // Erin attempts to redeem 400 LAI
    const {
      firstRedemptionHint,
      partialRedemptionHintNICR
    ...
    // Check activePool debt reduced by  400 LAI
    const activePool_debt_after = await activePool.getLAIDebt()
    ...
    const erin_balance_after = (await laiToken.balanceOf(erin)).toString()
    assert.equal(erin_balance_after, '0')
  })

  it("redeemCollateral(): reverts when requested redemption amount exceeds caller's LAI token balance", async () => {
    const { collateral: W_coll, totalDebt: W_totalDebt } = await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: whale } })
    ...
    try {
      ...
      assert.include(error.message, "Requested redemption amount must be <= user's LAI token balance")
    }

    // Erin tries to redeem 401 LAI
    try {
      ({
        firstRedemptionHint,
        partialRedemptionHintNICR
    ...
    // Erin tries to redeem 239482309 LAI
    try {
      ({
        firstRedemptionHint,
        partialRedemptionHintNICR
    ...
    // Erin tries to redeem 2^256 - 1 LAI
  ...
  it("redeemCollateral(): value of issued ETH == face value of redeemed LAI (assuming 1 LAI has value of $1)", async () => {
    const { collateral: W_coll } = await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: whale } })

    // Alice opens trove and transfers 1000 LAI each to Erin, Flyn, Graham
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraLAIAmount: dec(4990, 18), extraParams: { from: alice } })
    await laiToken.transfer(erin, dec(1000, 18), { from: alice })
    await laiToken.transfer(flyn, dec(1000, 18), { from: alice })
    await laiToken.transfer(graham, dec(1000, 18), { from: alice })

    // B, C, D open trove
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(300, 16)), extraLAIAmount: dec(1590, 18), extraParams: { from: bob } })
    const { collateral: C_coll } = await openTrove({ ICR: toBN(dec(600, 16)), extraLAIAmount: dec(1090, 18), extraParams: { from: carol } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(800, 16)), extraLAIAmount: dec(1090, 18), extraParams: { from: dennis } })

    const totalColl = W_coll.add(A_coll).add(B_coll).add(C_coll).add(D_coll)

    const price = await priceFeed.getPrice()

    const _120_LAI = '120000000000000000000'
    const _373_LAI = '373000000000000000000'
    const _950_LAI = '950000000000000000000'
    ...
    // Erin redeems 120 LAI
    ({
      firstRedemptionHint,
      partialRedemptionHintNICR
    } = await hintHelpers.getRedemptionHints(_120_LAI, price, 0))
    ...
    /* 120 LAI redeemed.  Expect $120 worth of ETH removed. At ETH:USD price of $200, 
    ETH removed = (120/200) = 0.6 ETH
    Total active ETH = 280 - 0.6 = 279.4 ETH */

    const activeETH_1 = await activePool.getETH()
    assert.equal(activeETH_1.toString(), activeETH_0.sub(toBN(_120_LAI).mul(mv._1e18BN).div(price)));

    // Flyn redeems 373 LAI
    ({
      firstRedemptionHint,
      partialRedemptionHintNICR
    } = await hintHelpers.getRedemptionHints(_373_LAI, price, 0))
    ...
    /* 373 LAI redeemed.  Expect $373 worth of ETH removed. At ETH:USD price of $200, 
    ETH removed = (373/200) = 1.865 ETH
    Total active ETH = 279.4 - 1.865 = 277.535 ETH */
    const activeETH_2 = await activePool.getETH()
    assert.equal(activeETH_2.toString(), activeETH_1.sub(toBN(_373_LAI).mul(mv._1e18BN).div(price)));

    // Graham redeems 950 LAI
    ({
      firstRedemptionHint,
      partialRedemptionHintNICR
    } = await hintHelpers.getRedemptionHints(_950_LAI, price, 0))
    ...
    /* 950 LAI redeemed.  Expect $950 worth of ETH removed. At ETH:USD price of $200, 
    ETH removed = (950/200) = 4.75 ETH
    Total active ETH = 277.535 - 4.75 = 272.785 ETH */
    const activeETH_3 = (await activePool.getETH()).toString()
    assert.equal(activeETH_3.toString(), activeETH_2.sub(toBN(_950_LAI).mul(mv._1e18BN).div(price)));
  })
  ...
  it("redeemCollateral(): reverts if there is zero outstanding system debt", async () => {
    // --- SETUP --- illegally mint LAI to Bob
    await laiToken.unprotectedMint(bob, dec(100, 18))

    assert.equal((await laiToken.balanceOf(bob)), dec(100, 18))
    ...
    // Bob tries to redeem his illegally obtained LAI
    try {
      const redemptionTx = await troveManager.redeemCollateral(
        dec(100, 18),
        firstRedemptionHint,
  ...
  it("redeemCollateral(): reverts if caller's tries to redeem more than the outstanding system debt", async () => {
    // --- SETUP --- illegally mint LAI to Bob
    await laiToken.unprotectedMint(bob, '101000000000000000000')

    assert.equal((await laiToken.balanceOf(bob)), '101000000000000000000')

    const { collateral: C_coll, totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(1000, 16)), extraLAIAmount: dec(40, 18), extraParams: { from: carol } })
    const { collateral: D_coll, totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(1000, 16)), extraLAIAmount: dec(40, 18), extraParams: { from: dennis } })

    const totalDebt = C_totalDebt.add(D_totalDebt)
    th.assertIsApproximatelyEqual((await activePool.getLAIDebt()).toString(), totalDebt)
    ...
    // Bob attempts to redeem his ill-gotten 101 LAI, from a system that has 100 LAI outstanding debt
    try {
      const redemptionTx = await troveManager.redeemCollateral(
  ...
  it("redeemCollateral(): a redemption made when base rate is zero increases the base rate", async () => {
    ...
    await openTrove({ ICR: toBN(dec(200, 16)), extraLAIAmount: dec(100, 18), extraParams: { from: A } })
    await openTrove({ ICR: toBN(dec(190, 16)), extraLAIAmount: dec(100, 18), extraParams: { from: B } })
    await openTrove({ ICR: toBN(dec(180, 16)), extraLAIAmount: dec(100, 18), extraParams: { from: C } })
    ...
    const A_balanceBefore = await laiToken.balanceOf(A)

    await th.redeemCollateral(A, contracts, dec(10, 18))

    // Check A's balance has decreased by 10 LAI
    assert.equal(await laiToken.balanceOf(A), A_balanceBefore.sub(toBN(dec(10, 18))).toString())
  ...
  it("redeemCollateral(): a redemption made when base rate is non-zero increases the base rate, for negligible time passed", async () => {
    ...
    const A_balanceBefore = await laiToken.balanceOf(A)
    const B_balanceBefore = await laiToken.balanceOf(B)
    ...
    // Check B's balance has decreased by 10 LAI
    assert.equal(await laiToken.balanceOf(B), B_balanceBefore.sub(toBN(dec(10, 18))).toString())
  ...
  it("redeemCollateral(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation [ @skip-on-coverage ]", async () => {
    ...
    // Check A's balance has decreased by 10 LAI
    assert.equal(A_balanceBefore.sub(await laiToken.balanceOf(A)), dec(10, 18))
  ...
  it("redeemCollateral(): a redemption made at zero base rate send a non-zero ETHFee to LAO staking contract", async () => {
    // time fast-forwards 1 year, and multisig stakes 1 LAO
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
    await laoToken.approve(laoStaking.address, dec(1, 18), { from: multisig })
    await laoStaking.stake(dec(1, 18), { from: multisig })
    ...
    // Check LAO Staking contract balance before is zero
    const laoStakingBalance_Before = await web3.eth.getBalance(laoStaking.address)
    assert.equal(laoStakingBalance_Before, '0')
    ...
    // Check LAO Staking contract balance after is non-zero
    const laoStakingBalance_After = toBN(await web3.eth.getBalance(laoStaking.address))
    assert.isTrue(laoStakingBalance_After.gt(toBN('0')))
  })

  it("redeemCollateral(): a redemption made at zero base increases the ETH-fees-per-LAO-staked in LAO Staking contract", async () => {
    // time fast-forwards 1 year, and multisig stakes 1 LAO
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
    await laoToken.approve(laoStaking.address, dec(1, 18), { from: multisig })
    await laoStaking.stake(dec(1, 18), { from: multisig })
    ...
    // Check LAO Staking ETH-fees-per-LAO-staked before is zero
    ...
    // Check LAO Staking ETH-fees-per-LAO-staked after is non-zero
    const F_ETH_After = await laoStaking.F_ETH()
    assert.isTrue(F_ETH_After.gt('0'))
  })

  it("redeemCollateral(): a redemption made at a non-zero base rate send a non-zero ETHFee to LAO staking contract", async () => {
    // time fast-forwards 1 year, and multisig stakes 1 LAO
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
    await laoToken.approve(laoStaking.address, dec(1, 18), { from: multisig })
    await laoStaking.stake(dec(1, 18), { from: multisig })
    ...
    const laoStakingBalance_Before = toBN(await web3.eth.getBalance(laoStaking.address))
    ...
    // check LAO Staking balance has increased
    assert.isTrue(laoStakingBalance_After.gt(laoStakingBalance_Before))
  })

  it("redeemCollateral(): a redemption made at a non-zero base rate increases ETH-per-LAO-staked in the staking contract", async () => {
    // time fast-forwards 1 year, and multisig stakes 1 LAO
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
    await laoToken.approve(laoStaking.address, dec(1, 18), { from: multisig })
    await laoStaking.stake(dec(1, 18), { from: multisig })
  ...
  it("redeemCollateral(): a redemption sends the ETH remainder (ETHDrawn - ETHFee) to the redeemer", async () => {
    ...
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraLAIAmount: dec(100, 18), extraParams: { from: A } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(190, 16)), extraLAIAmount: dec(100, 18), extraParams: { from: B } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(180, 16)), extraLAIAmount: dec(100, 18), extraParams: { from: C } })
    ...
    // Check total LAI supply
    const activeLAI = await activePool.getLAIDebt()
    const defaultLAI = await defaultPool.getLAIDebt()

    const totalLAISupply = activeLAI.add(defaultLAI)
    th.assertIsApproximatelyEqual(totalLAISupply, totalDebt)
  ...
  it("redeemCollateral(): a full redemption (leaving trove with 0 debt), closes the trove", async () => {
    ...
    const { netDebt: W_netDebt } = await openTrove({ ICR: toBN(dec(20, 18)), extraLAIAmount: dec(10000, 18), extraParams: { from: whale } })

    const { netDebt: A_netDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraLAIAmount: dec(100, 18), extraParams: { from: A } })
    const { netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(190, 16)), extraLAIAmount: dec(100, 18), extraParams: { from: B } })
    const { netDebt: C_netDebt } = await openTrove({ ICR: toBN(dec(180, 16)), extraLAIAmount: dec(100, 18), extraParams: { from: C } })
    const { netDebt: D_netDebt } = await openTrove({ ICR: toBN(dec(280, 16)), extraLAIAmount: dec(100, 18), extraParams: { from: D } })
    ...
    // whale redeems 360 LAI.  Expect this to fully redeem A, B, C, and partially redeem D.
  ...
  const redeemCollateral3Full1Partial = async () => {
    ...
    const { netDebt: A_netDebt, collateral: A_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraLAIAmount: dec(100, 18), extraParams: { from: A } })
    const { netDebt: B_netDebt, collateral: B_coll } = await openTrove({ ICR: toBN(dec(190, 16)), extraLAIAmount: dec(100, 18), extraParams: { from: B } })
    const { netDebt: C_netDebt, collateral: C_coll } = await openTrove({ ICR: toBN(dec(180, 16)), extraLAIAmount: dec(100, 18), extraParams: { from: C } })
    const { netDebt: D_netDebt } = await openTrove({ ICR: toBN(dec(280, 16)), extraLAIAmount: dec(100, 18), extraParams: { from: D } })
    const redemptionAmount = A_netDebt.add(B_netDebt).add(C_netDebt).add(toBN(dec(10, 18)))
    ...
    // whale redeems LAI.  Expect this to fully redeem A, B, C, and partially redeem D.
    await th.redeemCollateral(whale, contracts, redemptionAmount)
  ...
  it("redeemCollateral(): emits correct debt and coll values in each redeemed trove's TroveUpdated event", async () => {
    ...
    const { netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(190, 16)), extraLAIAmount: dec(100, 18), extraParams: { from: B } })
    const { netDebt: C_netDebt } = await openTrove({ ICR: toBN(dec(180, 16)), extraLAIAmount: dec(100, 18), extraParams: { from: C } })
    const { totalDebt: D_totalDebt, collateral: D_coll } = await openTrove({ ICR: toBN(dec(280, 16)), extraLAIAmount: dec(100, 18), extraParams: { from: D } })
    ...
    // whale redeems LAI.  Expect this to fully redeem A, B, C, and partially redeem 15 LAI from D.
  ...
  it("redeemCollateral(): a redemption that closes a trove leaves the trove's ETH surplus (collateral - ETH drawn) available for the trove owner after re-opening trove", async () => {
    ...
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraLAIAmount: dec(100, 18), extraParams: { from: A } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(190, 16)), extraLAIAmount: dec(100, 18), extraParams: { from: B } })
    const { collateral: C_coll } = await openTrove({ ICR: toBN(dec(180, 16)), extraLAIAmount: dec(100, 18), extraParams: { from: C } })
  ...
  it('redeemCollateral(): reverts if fee eats up all returned collateral', async () => {
    // --- SETUP ---
    const { laiAmount } = await openTrove({ ICR: toBN(dec(200, 16)), extraLAIAmount: dec(1, 24), extraParams: { from: alice } })
    ...
    for (let i = 0; i < 2; i++) {
      ...
      const {
        firstRedemptionHint,
        partialRedemptionHintNICR
      } = await hintHelpers.getRedemptionHints(laiAmount, price, 0)
      ...
      await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: bob } })
      await borrowerOperations.adjustTrove(th._100pct, 0, laiAmount, true, alice, alice, { from: alice, value: laiAmount.mul(mv._1e18BN).div(price) })
    }
  ...
  it("getPendingLAIDebtReward(): Returns 0 if there is no pending LAIDebt reward", async () => {
    // Make some troves
    const { totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extraLAIAmount: dec(100, 18), extraParams: { from: defaulter_1 } })

    await openTrove({ ICR: toBN(dec(3, 18)), extraLAIAmount: dec(20, 18), extraParams: { from: carol } })

    await openTrove({ ICR: toBN(dec(20, 18)), extraLAIAmount: totalDebt, extraParams: { from: whale } })
    ...
    const current_L_LAIDebt = await troveManager.L_LAIDebt()
    assert.equal(current_L_LAIDebt, 0)

    const carolSnapshot_L_LAIDebt = (await troveManager.rewardSnapshots(carol))[1]
    assert.equal(carolSnapshot_L_LAIDebt, 0)

    const carol_PendingLAIDebtReward = await troveManager.getPendingLAIDebtReward(carol)
    assert.equal(carol_PendingLAIDebtReward, 0)
  })
  ...
  it("computeICR(): Returns 2^256-1 for ETH:USD = 100, coll = 1 ETH, debt = 100 LAI", async () => {
    const price = dec(100, 18)
    const coll = dec(1, 'ether')
    const debt = dec(100, 18)
  ...
  it("computeICR(): returns correct ICR for ETH:USD = 100, coll = 200 ETH, debt = 30 LAI", async () => {
    const price = dec(100, 18)
  ...
  it("computeICR(): returns correct ICR for ETH:USD = 250, coll = 1350 ETH, debt = 127 LAI", async () => {
  ...
  it("computeICR(): returns correct ICR for ETH:USD = 100, coll = 1 ETH, debt = 54321 LAI", async () => {
    const price = dec(100, 18)
    const coll = dec(1, 'ether')
  ...
  it("getTroveStatus(): Returns status", async () => {
    ...
    await openTrove({ ICR: toBN(dec(150, 16)), extraLAIAmount: B_totalDebt, extraParams: { from: A } })

    // to be able to repay:
    await laiToken.transfer(B, B_totalDebt, { from: A })
