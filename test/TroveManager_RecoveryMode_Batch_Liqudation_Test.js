...
contract('TroveManager - in Recovery Mode - back to normal mode in 1 tx', async accounts => {
  ...
  context('Batch liquidations', () => {
    const setup = async () => {
      ...
      await openTrove({ ICR: toBN(dec(340, 16)), extraLAIAmount: totalLiquidatedDebt, extraParams: { from: whale } })
    ...
    it('Stability Pool profit matches', async () => {
      ...
      const spLaiBefore = await stabilityPool.getTotalLAIDeposits()
      ...
      const spLaiAfter = await stabilityPool.getTotalLAIDeposits()
      ...
      const expectedGainInLAI = expectedCollateralLiquidatedA.mul(price).div(mv._1e18BN).sub(A_totalDebt)
      const realGainInLAI = spEthAfter.sub(spEthBefore).mul(price).div(mv._1e18BN).sub(spLaiBefore.sub(spLaiAfter))

      assert.equal(spEthAfter.sub(spEthBefore).toString(), expectedCollateralLiquidatedA.toString(), 'Stability Pool ETH doesn’t match')
      assert.equal(spLaiBefore.sub(spLaiAfter).toString(), A_totalDebt.toString(), 'Stability Pool LAI doesn’t match')
      assert.equal(realGainInLAI.toString(), expectedGainInLAI.toString(), 'Stability Pool gains don’t match')
    })
    ...
    it('A trove over TCR is not liquidated', async () => {
      ...
      await openTrove({ ICR: toBN(dec(310, 16)), extraLAIAmount: totalLiquidatedDebt, extraParams: { from: whale } })
      await stabilityPool.provideToSP(totalLiquidatedDebt, ZERO_ADDRESS, { from: whale })
  ...
  context('Sequential liquidations', () => {
    const setup = async () => {
      ...
      await openTrove({ ICR: toBN(dec(300, 16)), extraLAIAmount: totalLiquidatedDebt, extraParams: { from: whale } })
      await stabilityPool.provideToSP(totalLiquidatedDebt, ZERO_ADDRESS, { from: whale })
