...
contract('Deploying and funding One Year Lockup Contracts', async accounts => {
  ...
  describe('Deploying LCs', async accounts => {
    ...
    it("LAO Deployer can deploy LCs directly", async () => {
      ...
      const LC_B = await LockupContract.new(laoToken.address, B, oneYearFromSystemDeployment, { from: liquityAG })
      const LC_B_txReceipt = await web3.eth.getTransactionReceipt(LC_B.transactionHash)

      const LC_C = await LockupContract.new(laoToken.address, C, oneYearFromSystemDeployment, { from: liquityAG })
    ...
    it("Anyone can deploy LCs directly", async () => {
      ...
      const LC_B = await LockupContract.new(laoToken.address, B, oneYearFromSystemDeployment, { from: E })
      const LC_B_txReceipt = await web3.eth.getTransactionReceipt(LC_B.transactionHash)

      const LC_C = await LockupContract.new(laoToken.address, C, oneYearFromSystemDeployment, { from: F })
      const LC_C_txReceipt = await web3.eth.getTransactionReceipt(LC_C.transactionHash)
    ...
    it("Direct deployment of LC sets the unlockTime in the LC", async () => {
      ...
      const LC_A = await LockupContract.new(laoToken.address, A, oneYearFromSystemDeployment, { from: liquityAG })
      const LC_B = await LockupContract.new(laoToken.address, B, '230582305895235', { from: B })
      const LC_C = await LockupContract.new(laoToken.address, C, dec(20, 18), { from: E })
    ...
    it("Direct deployment of LC reverts when the unlockTime is < 1 year from system deployment", async () => {
      ...
      const LCDeploymentPromise_A = LockupContract.new(laoToken.address, A, nearlyOneYear, { from: liquityAG })
      const LCDeploymentPromise_B = LockupContract.new(laoToken.address, B, '37', { from: B })
      const LCDeploymentPromise_C = LockupContract.new(laoToken.address, C, '43200', { from: E })
  ...
  describe('Funding LCs', async accounts => {
    it("LAO transfer from LAO deployer to their deployed LC increases the LAO balance of the LC", async () => {
      // Deploy 5 LCs
      const deployedLCtx_A = await lockupContractFactory.deployLockupContract(A, oneYearFromSystemDeployment, { from: liquityAG })
      const deployedLCtx_B = await lockupContractFactory.deployLockupContract(B, oneYearFromSystemDeployment, { from: liquityAG })
      const deployedLCtx_C = await lockupContractFactory.deployLockupContract(C, oneYearFromSystemDeployment, { from: liquityAG })
      ...
      assert.equal(await laoToken.balanceOf(LCAddress_D), '0')
      assert.equal(await laoToken.balanceOf(LCAddress_E), '0')

      // Multisig transfers LAO to each LC
      await laoToken.transfer(LCAddress_A, LAOEntitlement_A, { from: multisig })
      await laoToken.transfer(LCAddress_B, LAOEntitlement_B, { from: multisig })
      await laoToken.transfer(LCAddress_C, LAOEntitlement_C, { from: multisig })
      await laoToken.transfer(LCAddress_D, LAOEntitlement_D, { from: multisig })
      await laoToken.transfer(LCAddress_E, LAOEntitlement_E, { from: multisig })

      assert.equal(await laoToken.balanceOf(LCAddress_A), LAOEntitlement_A)
      assert.equal(await laoToken.balanceOf(LCAddress_B), LAOEntitlement_B)
      assert.equal(await laoToken.balanceOf(LCAddress_C), LAOEntitlement_C)
      assert.equal(await laoToken.balanceOf(LCAddress_D), LAOEntitlement_D)
      assert.equal(await laoToken.balanceOf(LCAddress_E), LAOEntitlement_E)
    })

    it("LAO Multisig can transfer LAO to LCs deployed through the factory by anyone", async () => {  
      ...
      // Multisig transfers LAO to each LC
      await laoToken.transfer(LCAddress_A, dec(1, 18), { from: multisig })
      await laoToken.transfer(LCAddress_B, dec(2, 18), { from: multisig })
      await laoToken.transfer(LCAddress_C, dec(3, 18), { from: multisig })
      await laoToken.transfer(LCAddress_D, dec(4, 18), { from: multisig })
      await laoToken.transfer(LCAddress_E, dec(5, 18), { from: multisig })

      assert.equal(await laoToken.balanceOf(LCAddress_A), dec(1, 18))
      assert.equal(await laoToken.balanceOf(LCAddress_B), dec(2, 18))
      assert.equal(await laoToken.balanceOf(LCAddress_C), dec(3, 18))
      assert.equal(await laoToken.balanceOf(LCAddress_D), dec(4, 18))
      assert.equal(await laoToken.balanceOf(LCAddress_E), dec(5, 18))
    })

    // can't transfer LAO to any LCs that were deployed directly
  })
  ...
  describe('Withdrawal attempts on funded, inactive LCs immediately after funding', async accounts => {
    it("Beneficiary can't withdraw from their funded LC", async () => {
      ...
      // Multisig transfers LAO to each LC
      await laoToken.transfer(LC_A.address, LAOEntitlement_A, { from: multisig })
      await laoToken.transfer(LC_B.address, LAOEntitlement_B, { from: multisig })
      await laoToken.transfer(LC_C.address, LAOEntitlement_C, { from: multisig })

      assert.equal(await laoToken.balanceOf(LC_A.address), LAOEntitlement_A)
      assert.equal(await laoToken.balanceOf(LC_B.address), LAOEntitlement_B)
      assert.equal(await laoToken.balanceOf(LC_C.address), LAOEntitlement_C)
      ...
      for (LC of LCs) {
        try {
          ...
          const withdrawalAttemptTx = await LC.withdrawLAO({ from: beneficiary })
    ...
    it("LAO multisig can't withraw from a LC which it funded", async () => {
      // Deploy 3 LCs
      const deployedLCtx_A = await lockupContractFactory.deployLockupContract(A, oneYearFromSystemDeployment, { from: liquityAG })
      const deployedLCtx_B = await lockupContractFactory.deployLockupContract(B, oneYearFromSystemDeployment, { from: liquityAG })
      const deployedLCtx_C = await lockupContractFactory.deployLockupContract(C, oneYearFromSystemDeployment, { from: liquityAG })
      ...
      // LAO multisig attempts to withdraw from LCs
      for (LC of LCs) {
        try {
          const withdrawalAttemptTx = await LC.withdrawLAO({ from: multisig })
          assert.isFalse(withdrawalAttemptTx.receipt.status)
        } catch (error) {
          assert.include(error.message, "revert")
        }
    ...
    it("No one can withraw from a LC", async () => {
      // Deploy 3 LCs
      const deployedLCtx_A = await lockupContractFactory.deployLockupContract(A, LAOEntitlement_A, { from: D })

      // Grab contract objects from deployment tx events
      const LC_A = await th.getLCFromDeploymentTx(deployedLCtx_A)

      // LiquityAG transfers LAO to the LC
      await laoToken.transfer(LC_A.address, LAOEntitlement_A, { from: multisig })
      ...
      try {
        const withdrawalAttemptTx = await LC_A.withdrawLAO({ from: G })
        assert.isFalse(withdrawalAttemptTx.receipt.status)
      } catch (error) {
        assert.include(error.message, "revert")
      }
      ...
      try {
        const withdrawalAttemptTx = await LC_A.withdrawLAO({ from: H })
        assert.isFalse(withdrawalAttemptTx.receipt.status)
      } catch (error) {
        assert.include(error.message, "revert")
      }
      ...
      try {
        const withdrawalAttemptTx = await LC_A.withdrawLAO({ from: I })
        assert.isFalse(withdrawalAttemptTx.receipt.status)
      } catch (error) {
        assert.include(error.message, "revert")
      }
