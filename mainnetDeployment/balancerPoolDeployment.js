...
const LAI = '0x5f98805A4E8be255a32880FDeC7F6728C6568bA0';
const CHAINLINK_ETHUSD_PROXY = '0x5f4eC3Df9cbd43714FE2740f5E3616155c5b8419';

const tokens = [LAI, WETH];
const weights = [toBigNum(dec(4, 17)), toBigNum(dec(6, 17))];

const NAME = 'WETH/LAI Pool';
const SYMBOL = '60WETH-40LAI';
...
async function main() {
  ...
  */
  ...
  const lai_balance = INITIAL_FUNDING.mul(weights[0]).div(toBigNum(dec(1, 18)));
  const initialBalances = [lai_balance, weth_balance];
  th.logBN('Initial LAI', lai_balance);
  ...
  // Approve LAI
  const lai = new ethers.Contract(
    LAI,
    ERC20.abi,
    deployerWallet
  );
  const txApproveLai = await lai.approve(VAULT, lai_balance);
  const approveLaiReceipt = await txApproveLai.wait();
  console.log('Approve LAI gas: ', approveLaiReceipt.gasUsed.toString());
  ...
  console.log('Total gas: ', receipt1.gasUsed.add(depositWethReceipt.gasUsed).add(approveWethReceipt.gasUsed).add(approveLaiReceipt.gasUsed).add(receipt2.gasUsed).toString());
}
