...
def get_lai_amount_from_net_debt(contracts, net_debt):
...
def logGlobalState(contracts):
    ...
    activePoolDebt = contracts.activePool.getLAIDebt()
    defaultPoolColl = contracts.defaultPool.getETH()
    defaultPoolDebt = contracts.defaultPool.getLAIDebt()
    ...
    SP_LAI = contracts.stabilityPool.getTotalLAIDeposits().to("ether")
    ...
    return [ETH_price, num_troves, total_coll, total_debt, TCR, recovery_mode, last_ICR, SP_LAI, SP_ETH]
