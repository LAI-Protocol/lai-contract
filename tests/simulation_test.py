...
def setAddresses(contracts):
    ...
    contracts.troveManager.setAddresses(
        ...
        contracts.laoToken.address,
        contracts.laoStaking.address,
        { 'from': accounts[0] }
    )
    ...
    # LAO
    contracts.laoStaking.setAddresses(
        contracts.laoToken.address,
        contracts.laiToken.address,
        contracts.troveManager.address,
        contracts.borrowerOperations.address,
...
def contracts():
    ...
    contracts.laiToken = LAIToken.deploy(
        contracts.troveManager.address,
        contracts.stabilityPool.address,
        contracts.borrowerOperations.address,
        { 'from': accounts[0] }
    ...
    # LAO
    contracts.laoStaking = LAOStaking.deploy({ 'from': accounts[0] })
    ...
    contracts.laoToken = LAOToken.deploy(
        contracts.communityIssuance.address,
        contracts.laoStaking.address,
...
def print_expectations():
    ...
    print("Expected LAO price at the end of first month: $", price_LAO_initial * (1 + drift_LAO)**720)
...
* trove liquidation
* return of the previous period's stability pool determined (liquidation gain & airdropped LAO gain)
* trove closure
* trove adjustment
* open troves
* issuance fee
* trove pool formed
* LAI supply determined
* LAI stability pool demand determined
* LAI liquidity pool demand determined
* LAI price determined
* redemption & redemption fee
* LAO pool return determined
"""
def test_run_simulation(add_accounts, contracts, print_expectations):
    LAI_GAS_COMPENSATION = contracts.troveManager.LAI_GAS_COMPENSATION() / 1e18
    ...
    with open('tests/simulation.csv', 'w', newline='') as csvfile:
        ...
        datawriter.writerow(['iteration', 'ETH_price', 'price_LAI', 'price_LAO', 'num_troves', 'total_coll', 'total_debt', 'TCR', 'recovery_mode', 'last_ICR', 'SP_LAI', 'SP_ETH', 'total_coll_added', 'total_coll_liquidated', 'total_lai_redempted'])
        ...
        for index in range(1, n_sim):
            ...
            result_liquidation = liquidate_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_LAI, price_LAO_current, data, index)
            total_coll_liquidated = total_coll_liquidated + result_liquidation[0]
            return_stability = result_liquidation[1]

            #close troves
            result_close = close_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_LAI, index)

            #adjust troves
            [coll_added_adjust, issuance_LAI_adjust] = adjust_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, index)

            #open troves
            [coll_added_open, issuance_LAI_open] = open_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_LAI, index)
            ...
            [price_LAI, redemption_pool, redemption_fee, issuance_LAI_stabilizer] = price_stabilizer(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_LAI, index)
            total_lai_redempted = total_lai_redempted + redemption_pool
            print('LAI price', price_LAI)
            print('LAO price', price_LAO_current)

            issuance_fee = price_LAI * (issuance_LAI_adjust + issuance_LAI_open + issuance_LAI_stabilizer)
            data['issuance_fee'][index] = issuance_fee
            data['redemption_fee'][index] = redemption_fee

            #LAO Market
            result_LAO = LAO_market(index, data)
            price_LAO_current = result_LAO[0]
            #annualized_earning = result_LAO[1]
            #MC_LAO_current = result_LAO[2]

            [ETH_price, num_troves, total_coll, total_debt, TCR, recovery_mode, last_ICR, SP_LAI, SP_ETH] = logGlobalState(contracts)
            print('Total redempted ', total_lai_redempted)
            ...
            datawriter.writerow([index, ETH_price, price_LAI, price_LAO_current, num_troves, total_coll, total_debt, TCR, recovery_mode, last_ICR, SP_LAI, SP_ETH, total_coll_added, total_coll_liquidated, total_lai_redempted])
