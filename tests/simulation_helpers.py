...
LAI_GAS_COMPENSATION = 200.0
MIN_NET_DEBT = 1800.0
MAX_FEE = Wei(1e18)

"""# Ether price (exogenous)

Ether is the collateral for LAI. The ether price $P_t^e$ follows
...
"""# LAO price
In the first month, the price of LAO follows

> $P_t^q = P_{t-1}^q (1+\zeta_t^q)(1+\sigma_t^q)$. 

Note that $\zeta_t^q \sim N(0,$ sd_LAO) represents LAO price shock and $\sigma_t^q$ the drift. Here, $\sigma_t^q =$ drift_LAO, so that the expected LAO price increases from price_LAO_initial to the following at the end of the first month:
> $E(P_{720}^q) = $price_LAO_initial$ \cdot (1+$ drift_LAO$)^{720}$

The LAO price from the second month on is endogenously determined.
"""

#LAO price & airdrop
price_LAO_initial = 0.4
price_LAO = [price_LAO_initial]
sd_LAO=0.005
drift_LAO = 0.0035
supply_LAO=[0]
LAO_total_supply=100000000

"""**LAO Endogenous Price**

The staked LAO pool earning consists of the issuance fee revenue and redemption fee revenue
> $R_t^q = R_t^i + R_t^r.$
...
> $$r_t=\frac{LAO Market Cap}{Annualized Earning}=\frac{MC_t}{E_t}$$

> $MC_t=P_t^q \cdot$ LAO_total_supply

Therefore, the LAO price dynamics is determined
> $$P_t^q=discount \cdot \frac{r^{PE}}{LAO\_total\_supply}E_t$$

Interpretation: The denominator implies that with more LAO tokens issued, LAO price decreases. However, the depreciation effect can be counteracted by the growth of the earning.
...
$R_{t-1}^s$ is the return in the stability pool, which consists of liquidation gain and airdrop LAO gain.
...
> (collateral in ether, debt in LAI, collateral ratio target, rational inattention, collateral ratio)
...
$P_{t}^{l}$ is the price of LAI.

$N_t^o$ is rounded to an integer.

---

The amount of LAI tokens generated by a new trove is
...
#sensitivity to LAI price & issuance fee
alpha = 0.3
...
- $R_t^a=P_{t}^q\hat{Q}_t^q$ is the airdrop gain, $\hat{Q}_t^q=1000$ denotes the amount of LAO token airdropped to the stability pool providers
- $D_{t}^{s}$ is the total amount of LAI deposited in the Stability Pool (see below)
...
"""LAO Price - First Month"""

#LAO price
for i in range(1, month):
    random.seed(2+13*i)
    shock_LAO = random.normalvariate(0,sd_LAO)
    price_LAO.append(price_LAO[i-1]*(1+shock_LAO)*(1+drift_LAO))
...
def pending_liquidations(contracts, price_ether_current):
    ...
    stability_pool_balance = contracts.stabilityPool.getTotalLAIDeposits()
    trove = last_trove
...
# F = 0.5 ** (1/8760)
# F = 0.99992087674
def quantity_LAO_airdrop(index):
    F = 0.99992087674
    if index <= 0:
        return 0
    return 32e6 * (F ** (index-1) - F ** index)

def liquidate_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_LAI, price_LAO_current, data, index):
    if len(active_accounts) == 0:
        return [0, 0]

    stability_pool_previous = contracts.stabilityPool.getTotalLAIDeposits() / 1e18
    ...
    while pending_liquidations(contracts, price_ether_current):
        try:
            ...
            for i in range(NUM_LIQUIDATIONS):
                ...
                ICR = contracts.troveManager.getCurrentICR(trove, Wei(price_ether_current * 1e18))
                print(f"ICR: {ICR}")
    stability_pool_current = contracts.stabilityPool.getTotalLAIDeposits() / 1e18
    ...
    liquidation_gain = ether_liquidated * price_ether_current - debt_liquidated * price_LAI
    airdrop_gain = price_LAO_current * quantity_LAO_airdrop(index)

    data['liquidation_gain'][index] = liquidation_gain
    data['airdrop_gain'][index] = airdrop_gain

    return_stability = calculate_stability_return(contracts, price_LAI, data, index)

    return [ether_liquidated, return_stability]

def calculate_stability_return(contracts, price_LAI, data, index):
    stability_pool_previous = contracts.stabilityPool.getTotalLAIDeposits() / 1e18
    if index == 0:
        ...
        return_stability = (year/index) * \
            (sum(data['liquidation_gain'][0:index]) +
             ...
             ) / (price_LAI * stability_pool_previous)
    else:
...
def close_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_LAI, index):
    if len(active_accounts) == 0:
        return [0]
    ...
    if index <= 240:
        ...
        number_closetroves = max(0, n_steady * (1+shock_closetroves)) + beta*(1-price_LAI)*n_troves
    ...
    for i in range(0, len(drops)):
        ...
        coll = amounts['coll']
        debt = amounts['debt']
        pending = get_lai_to_repay(accounts, contracts, active_accounts, inactive_accounts, account, debt)
        if pending == 0:
...
def transfer_from_to(contracts, from_account, to_account, amount):
    balance = contracts.laiToken.balanceOf(from_account)
    transfer_amount = min(balance, amount)
    if transfer_amount == 0:
        return amount
    if from_account == to_account:
        return amount
    contracts.laiToken.transfer(to_account, transfer_amount, { 'from': from_account })
    pending = amount - transfer_amount

    return pending

def get_lai_to_repay(accounts, contracts, active_accounts, inactive_accounts, account, debt):
    laiBalance = contracts.laiToken.balanceOf(account)
    if debt > laiBalance:
        pending = debt - laiBalance
        ...
        # try with active accounts, which are more likely to hold LAI
        ...
        if pending > 0:
            print(f"\n ***Error: not enough LAI to repay! {debt / 1e18} LAI for {account}")
...
def adjust_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, index):
    ...
    for i, working_trove in enumerate(active_accounts):
        ...
        if p >= ratio:
            ...
            if check < -1:
                ...
                pending = get_lai_to_repay(accounts, contracts, active_accounts, inactive_accounts, account, repay_amount)
                if pending == 0:
                    contracts.borrowerOperations.repayLAI(repay_amount, hints[0], hints[1], { 'from': account })
                ...
                if isNewTCRAboveCCR(contracts, 0, False, withdraw_amount_wei, True, floatToWei(price_ether_current)):
                    contracts.borrowerOperations.withdrawLAI(MAX_FEE, withdraw_amount_wei, hints[0], hints[1], { 'from': account })
                    rate_issuance = contracts.troveManager.getBorrowingRateWithDecay() / 1e18
                    issuance_LAI_adjust = issuance_LAI_adjust + rate_issuance * withdraw_amount
    ...
    return [coll_added_float, issuance_LAI_adjust]
...
def open_trove(accounts, contracts, active_accounts, inactive_accounts, supply_trove, quantity_ether, CR_ratio, rational_inattention, price_ether_current):
    ...
    debtChange = floatToWei(supply_trove) + LAI_GAS_COMPENSATION
    lai = get_lai_amount_from_net_debt(contracts, floatToWei(supply_trove))
    if isNewTCRAboveCCR(contracts, coll, True, debtChange, True, floatToWei(price_ether_current)):
        contracts.borrowerOperations.openTrove(MAX_FEE, lai, hints[0], hints[1],
...
def open_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_LAI, index):
    ...
    elif price_LAI <= 1 + rate_issuance:
        number_opentroves = max(0, n_steady * (1+shock_opentroves))
    else:
        number_opentroves = max(0, n_steady * (1+shock_opentroves)) + \
                        alpha * (price_LAI - rate_issuance - 1) * n_troves
    ...
    for i in range(0, number_opentroves):
        ...
        issuance_LAI_open = issuance_LAI_open + rate_issuance * supply_trove
    ...
    return [coll_added, issuance_LAI_open]
...
def stability_update(accounts, contracts, active_accounts, return_stability, index):
    supply = contracts.laiToken.totalSupply() / 1e18
    stability_pool_previous = contracts.stabilityPool.getTotalLAIDeposits() / 1e18
    ...
    if stability_pool > stability_pool_previous:
        ...
        while remaining > 0 and i < len(active_accounts):
          ...
          balance = contracts.laiToken.balanceOf(account) / 1e18
        ...
        current_deposit = contracts.stabilityPool.getCompoundedLAIDeposit(accounts[0])
...
"""LAI Price, liquidity pool, and redemption

**Price Determination**

---
With the supply and demand of LAI tokens defined above, the price of LAI at the current period is given by the following equilibrium condition:
> $$S_t = D_t^s + D_t^l = D_t^s + D_{t-1}^l (1+\zeta_t^l)(1+\sigma_t^l) (\frac{P_t^l}{P_{t-1}^l})^\delta$$

where $S$ is the total supply of LAI.

Solving this equation gives that:
> $$P_t^l = P_{t-1}^l (\frac{S_t-D_t^s}{D_{t-1}^l(1+\zeta_t^l)(1+\sigma_t^l)})^{1/\delta}$$
"""

def calculate_price(price_LAI, liquidity_pool, liquidity_pool_next):
  # liquidity_pool = supply - stability_pool
  # liquidity_pool_next = liquidity_pool_previous * drift_liquidity * (1+shock_liquidity)
  price_LAI_current= price_LAI * (liquidity_pool / liquidity_pool_next) ** (1/delta)

  return price_LAI_current

""" **Stabilizers**

There are two stabilizers to attenuate LAI price deviation from its target range.
...
The amount of $Q_t^d(c)$ is expected to bring the LAI price back to $1.1+f_t^i$. This means that
...
If $P_t^l < 1-f_t^r$, a fraction $\chi_t$ of LAI in the liquidity pool is used for redemption
> $$D_t^r = \chi_t D_t^l,$$
...
Note that unlike stabilizer 1, stabilizer 2 has impact of LAI price in
...
def redeem_trove(accounts, contracts, i, price_ether_current):
    lai_balance = contracts.laiToken.balanceOf(accounts[i])
    [firstRedemptionHint, partialRedemptionHintNICR, truncatedLAIamount] = contracts.hintHelpers.getRedemptionHints(lai_balance, price_ether_current, 70)
    if truncatedLAIamount == Wei(0):
        return None
    ...
    try:
        ...
        print(f"LAI Token:    {contracts.laiToken.address}")
        print(f"i: {i}")
        print(f"account: {accounts[i]}")
        print(f"LAI bal: {lai_balance / 1e18}")
        print(f"truncated: {truncatedLAIamount / 1e18}")
        ...
        print(f"amount: {truncatedLAIamount}")
...
def price_stabilizer(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_LAI, index):

    stability_pool = contracts.stabilityPool.getTotalLAIDeposits() / 1e18
    redemption_pool = 0
    redemption_fee = 0
    issuance_LAI_stabilizer = 0
    ...
    #Calculating Price
    price_LAI_current = calculate_price(price_LAI, liquidity_pool, liquidity_pool_next)
    rate_issuance = contracts.troveManager.getBorrowingRateWithDecay() / 1e18
    rate_redemption = contracts.troveManager.getRedemptionRateWithDecay() / 1e18

    #Stabilizer
    #Ceiling Arbitrageurs
    if price_LAI_current > 1.1 + rate_issuance:
        supply_wanted = stability_pool + \
                        liquidity_pool_next * \
                        ((1.1+rate_issuance) / price_LAI)**delta
        ...
        issuance_LAI_stabilizer = rate_issuance * supply_trove
        if open_trove(accounts, contracts, active_accounts, inactive_accounts, supply_trove, quantity_ether, CR_ratio, rational_inattention):
            price_LAI_current = 1.1 + rate_issuance
            liquidity_pool = supply_wanted - stability_pool


    #Floor Arbitrageurs
    if price_LAI_current < 1 - rate_redemption:
        ...
        supply_target = stability_pool + \
                        liquidity_pool_next * \
                        ((1-rate_redemption) / price_LAI)**delta
        ...
        if supply_diff < redemption_ratio * liquidity_pool:
            ...
            price_LAI_current = 1 - rate_redemption
        else:
    ...
    return [price_LAI_current, redemption_pool, redemption_fee, issuance_LAI_stabilizer]

"""# LAO Market"""

def LAO_market(index, data):
    #quantity_LAO = (LAO_total_supply/3)*(1-0.5**(index/period))
    np.random.seed(2+3*index)
    if index <= month:
        price_LAO_current = price_LAO[index-1]
        ...
        price_LAO_current = discount * PE_ratio * annualized_earning / LAO_total_supply

    #MC_LAO_current = price_LAO_current * quantity_LAO

    return [price_LAO_current, annualized_earning]
